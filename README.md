...
## Escáner de fotos con IA

Esta versión agrega un backend mínimo para escanear fotos de figuritas con una API de visión. La API key queda en el servidor, no en el navegador.
...
Cuando subís una foto desde la tarjeta **Carga rápida**, el frontend llama a:

```text
POST /api/scan-stickers
```

El backend manda la imagen al modelo de visión, valida que los códigos existan en el álbum y devuelve una lista editable antes de confirmar.

> Importante: no abras `index.html` directo con doble click si querés usar el escáner con IA, porque necesita el backend corriendo.
