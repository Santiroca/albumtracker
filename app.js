...
function updateStats(){
  ...
  setTxt('smain-side', `${got}/${TOTAL}`);
  setTxt('ms-got-side', got);
  setTxt('ms-mis-side', miss);
  setTxt('ms-dup-side', dups);
...
function startLongPress(e, id){
  ...
  lpTimer = setTimeout(()=>{
    ...
    save(); updateStats();
    const c = col[id]||0;
    toast(c===0 ? `Quitaste ${id}` : `${id} ×${c}`, c===0?'err':'warn');
    renderGrid();
    if(isActive('dupes')) renderDupes();
...
function genQR(){
  ...
  const qrUrl = `https://santiroca.github.io/albumtracker/?code=${encodeURIComponent(payload)}`;

  function normalizeQRLayout(){
    const nodes = Array.from(box.children).filter(n => n.matches && n.matches('canvas,img,table'));
    if(!nodes.length) return;

    // qrcodejs can create both canvas and image fallback. Keep only one.
    const chosen = box.querySelector('canvas') || box.querySelector('img') || box.querySelector('table');
    nodes.forEach(n => {
      if(n !== chosen) n.remove();
    });

    chosen.classList.add('qr-main');
    chosen.style.display = 'block';
    chosen.style.margin = '0 auto';
    chosen.style.maxWidth = 'min(220px, 100%)';
    chosen.style.width = 'min(220px, 100%)';
  ...
  try{
    ...
    requestAnimationFrame(normalizeQRLayout);
  }catch(e){
    const dupesOnly = Object.keys(col).filter(id=>col[id]>1).map(encodeShortId).join(',');
    const fallbackPayload = 'p26:d=' + dupesOnly;
    const fallbackUrl = `https://santiroca.github.io/albumtracker/?code=${encodeURIComponent(fallbackPayload)}`;
    try{
      new QRCode(box,{text:fallbackUrl,width:220,height:220,colorDark:'#000000',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.L});
      requestAnimationFrame(normalizeQRLayout);
    }catch(e2){
      box.innerHTML='<div style="color:var(--red);font-size:.68rem;padding:14px;text-align:center;">Colección muy grande para QR.<br>Compartí por WhatsApp.</div>';
    }
  }
}
...
function toast(msg,type='info'){
  ...
  t.className = 'toast';
  t.textContent = msg;
  // Restart CSS transition even if another toast is still visible.
  void t.offsetWidth;
...
(function(){
  ...
  function calcTrack(){
    ...
    barH = Math.max(40, Math.min(trackH * (clientHeight / scrollHeight), trackH));
  }
  ...
  // Scroll updates bar position always, indicator only when dragging
