  ...
  try{
    if(video && canvas && video.readyState >= 2){
      ...
      if(window.jsQR){
        ...
        if(window._tradeScanMisses === 150){
...
function parseAnyImport(raw){
  ...
  if(/Figuritas App|Me faltan|Repetidas/i.test(text)){
    ...
    if(fig && fig.collection) return {collection: fig.collection, ignored: fig.unknown?.length || 0, source:'Figuritas'};
  }
...
function parseFiguritasList(raw){
  ...
  if(!text || !/Figuritas App|Me faltan|Repetidas/i.test(text)) return null;
  ...
  for(const rawLine of text.split('\n')){
    ...
    if(/^descarga\s+la\s+app/i.test(line) || /^https?:\/\//i.test(line)){
      section = null;
      continue;
    }
    if(!section) continue;

    const m = line.match(/^([A-Z]{2,4})\b[^:]*:\s*(.+)$/i);
    if(!m) continue;

    const prefix = m[1].toUpperCase();
    const nums = (m[2].match(/\d+/g) || []).map(n => parseInt(n, 10)).filter(Boolean);
    if(!nums.length) continue;

    parsedLines++;
    for(const n of nums){
      const id = figuritasId(prefix, n);
      if(!id) continue;
      ...
      if(section === 'missing') missing.add(up);
      else if(section === 'dupes') dupes.push(up);
    }
  }
  ...
  for(const id of ALL_IDS){
    const up = id.toUpperCase();
    if(!missing.has(up)) next[up] = 1;
  }

  for(const id of dupes){
    // Repetida means at least one pasted copy + one extra copy.
    next[id] = Math.max(next[id] || 1, 2);
  }
  ...
  return {
    ...
    importedCount: Object.keys(next).length,
...
function importFromFiguritas(){
  ...
  const collection = parsed.collection || parsed.col;
  const unknownCount = parsed.unknown ? parsed.unknown.length : 0;
  const note = `Detecté ${parsed.missingCount || 0} faltantes y ${parsed.dupesCount || 0} repetidas.${unknownCount ? ' ' + unknownCount + ' ignoradas.' : ''}`;
  showImportPreview(collection, 'Importar desde app "Figuritas"', note);
}
...
function parsePhotoImportText(text){
  ...
  const re = /\b(CC|FWC|[A-Z]{3})[\s-]?(\d{1,2})\b/g;
  let m;
  while((m = re.exec(raw))){
    const id = normalizePhotoImportId(m[1], m[2]);
    if(!id) continue;
    const up = id.toUpperCase();
    if(known.has(up)) found.push(up);
    else unknown.push(up);
  }
  ...
  return new Promise((resolve, reject)=>{
    ...
    img.onload = ()=>{
      try{
        ...
        // Preprocesado simple: blanco y negro con contraste.
...
async function readPhotoImportCode(file){
  ...
  photoImportStatus('Leyendo la foto… puede tardar unos segundos.', 'info');

  try{
    const Tesseract = await ensureTesseract();
    const canvas = await createOcrImage(file);
    ...
    if(parsed.found.length){
      const value = parsed.found.join(', ');
      if(textArea) textArea.value = value;
      previewPhotoImport();
      toast(`Detecté ${parsed.found.length} código${parsed.found.length===1?'':'s'}`,'ok');
      return;
    }

    photoImportStatus('No pude leer el código automáticamente. Escribilo abajo como TUN 18.', 'warn');
    if(textArea) textArea.focus();
  }catch(err){
    photoImportStatus('No pude activar la lectura automática. Escribí el código abajo como TUN 18.', 'warn');
    if(textArea) textArea.focus();
  }
}
...
function previewPhotoImport(){
  ...
  if(!parsed.found.length){
    foundBox.style.display = 'block';
    foundBox.innerHTML = '<div class="photo-import-empty">No encontré códigos. Probá escribirlos como ARG 10, MEX-13 o FWC 3.</div>';
  ...
  foundBox.style.display = 'block';
  foundBox.innerHTML = `
    <div class="photo-import-title">Se van a pegar ${parsed.found.length} figuritas</div>
    <div class="photo-import-chips">
      ${unique.map(id => `<span class="photo-chip">${id}${counts[id] > 1 ? ` ×${counts[id]}` : ''}</span>`).join('')}
    </div>
    ${parsed.unknown.length ? `<div class="photo-import-warn">${parsed.unknown.length} códigos ignorados porque no existen en el álbum.</div>` : ''}
  `;
...
function applyPhotoImport(){
  ...
  toast(`${total} figuritas pegadas`, 'ok', {undo:true});
}
