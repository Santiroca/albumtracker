...
const TUTORIAL_KEY = 'pn26_tutorial_seen_v6';
let tutorialStep = 0;
...
function ensureTutorialTargetVisible(target){
  ...
  try{
    ...
    if(isPhone){
      ...
      const freeTop = targetIsLower ? 96 : 110;
      const freeBottom = targetIsLower ? Math.round(vp.height * 0.52) : (vp.height - 210);
...
function positionTutorial(){
  ...
  if(target){
    ...
    if(isPhone){
      ...
      // If target is in the lower half, pin card at the top. Otherwise pin card at the bottom.
      const targetIsLower = rect.top > vh * 0.45;
      const topDock = 88;
      const bottomDock = 'max(10px, calc(10px + env(safe-area-inset-bottom)))';
      if(targetIsLower){
        card.style.top = `${topDock}px`;
        card.style.bottom = 'auto';
      } else {
        card.style.top = 'auto';
        card.style.bottom = bottomDock;
      }
      return;
    }
