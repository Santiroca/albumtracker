...
function showMatch(them){
  ...
  function renderChip(id, cls, container, selectedMap, countEl, maxQty){
    const info = STICKER_MAP[id];
    const availableLabel = (selectedMap === window._tradeGiveSelected && maxQty) ? ` · disp. ${maxQty}` : '';
    const baseLabel = info ? `${id.replace('-',' ')} · ${info.name}${availableLabel}` : `${id}${availableLabel}`;
