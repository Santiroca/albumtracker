...
function genQR(){
  ...
  function normalizeQRLayout(){
    const qrNode = box.querySelector('canvas, img, table');
    if(!qrNode) return;
    qrNode.style.display = 'block';
    qrNode.style.margin = '0 auto';
    qrNode.style.maxWidth = 'min(220px, 100%)';
    qrNode.style.width = 'min(220px, 100%)';
...
(function(){
  ...
  scroll.addEventListener('touchstart', ()=>{ updateBar(); updateIndicator(); }, {passive:true});
  ...
  function onDragEnd(){
    isDragging=false; bar.style.transition='opacity .25s ease';
    hideTimer = setTimeout(()=>bar.classList.remove('visible'), 1400);
    ind._hideTimer = setTimeout(()=>ind.classList.remove('visible'), 1400);
  }
  ...
  updateBar();
  setTimeout(()=>{ updateBar(); updateIndicator(); }, 250);
})();
