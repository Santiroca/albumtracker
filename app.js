...
(function(){
  ...
  function shouldUseScrollAid(){
    ...
    return panel === 'album' || panel === 'dupes' || panel === 'summary' || panel === 'config';
  }
  ...
  function getActiveHeaders(){
    ...
    if(panel === 'config'){
      ...
      document.querySelectorAll('#panel-config .settings-card').forEach(card => {
        ...
        const rawTitle = card.querySelector('.card-title')?.textContent || 'Configuración';
        const cleanTitle = rawTitle.replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}]/gu, '').trim();
        items.push({
          el: card,
          flag: '',
          abbrev: cleanTitle || 'Configuración'
        });
      });
      return items;
    }
...
const TUTORIAL_KEY = 'pn26_tutorial_seen_v12';
let tutorialStep = 0;
...
const tutorialSteps = [
  {
    ...
    text:'Cargá tu álbum, mirá el progreso y guardá amigos por código fijo.',
    ...
    hint:'Todo se guarda automáticamente en este dispositivo.'
  },
  {
    kicker:'Instalación',
    ...
    items:[
      ...
      ['🔄','Actualizar','Si no cambia, cerrá la app desde multitarea y abrila otra vez.']
    ],
    hint:'En iPhone usá Safari para instalarla.'
  },
  {
    kicker:'Álbum',
    title:'Cargar y buscar',
    text:'La pestaña Álbum es para marcar lo que tenés y encontrar figuritas rápido.',
    before:()=>goTab('album'),
  ...
  },
  {
    kicker:'Progreso',
    title:'Ver avances',
    text:'En Progreso ves cuánto completaste y qué equipos te faltan.',
    ...
    items:[
      ...
      ['🌎','Por equipo','Mirá qué países están completos o casi completos.'],
      ['↕️','Orden','Ordená para encontrar rápido lo importante.']
    ],
    hint:'Útil para saber qué buscar primero.'
  },
  {
    kicker:'Intercambio',
    title:'Código fijo y QR',
    text:'Tu QR y tu código fijo sirven para que un amigo te guarde una sola vez.',
    before:()=>goTab('trade'),
    target:'#qrbox',
    items:[
      ['🔳','Compartir','Mandá tu QR desde el botón Compartir.'],
      ['🔗','Copiar link','Abre la app con tu código ya cargado.'],
      ['🔁','Actualizado','Cuando cambies repetidas, tus amigos pueden traerlas nuevas.']
    ],
  ...
  },
  {
    ...
    text:'Escaneá un QR o pegá el código fijo de tu amigo para guardarlo.',
    before:()=>goTab('trade'),
    target:()=>document.querySelector('.online-add-row') || document.querySelector('.trade-friends-card'),
    items:[
      ['📷','Escanear QR','Usá cámara o una imagen de la galería.'],
      ['⌨️','Manual','Poné nombre, código fijo y Guardar amigo.'],
      ['🔵','Actualizar y ver repetidas','Trae sus repetidas nuevas cuando las necesites.']
    ],
  ...
  },
  {
    ...
    title:'Datos y ayuda',
    text:'Acá están los backups, importación, repetidas, actualización y borrar álbum.',
    before:()=>goTab('config'),
    target:'#panel-config',
    items:[
      ['📤','Backup','Exportá una copia antes de cambiar de celular.'],
      ['📥','Importar','Recuperá un backup o una lista de Figuritas.'],
      ['🗑️','Borrar álbum','Está al final para evitar tocarlo por error.']
    ],
    hint:'También podés volver a abrir este tutorial desde Configuración.'
  }
]
