...
function normalizeTradePayload(raw){
  ...
  // QRCode genera una URL tipo:
  // https://santiroca.github.io/albumtracker/?code=p26%3Ac%3D...
  // El scanner devuelve esa URL completa, por eso primero extraemos ?code=.
  try{
...
async function startTradeCam(){
  ...
  if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){
    stat.textContent = 'Tu navegador no permite cámara en vivo — usá galería o pegá el código';
    document.getElementById('qr-capture-input')?.click();
    return;
  }

  if(!window.jsQR && !window.BarcodeDetector){
    stat.textContent = 'Cargando lector QR… si no abre, usá galería o pegá el código';
  }

  try{
    stat.textContent = 'Abriendo cámara…';
    tradStream = await navigator.mediaDevices.getUserMedia({
      audio: false,
      video: {
    ...
    stat.textContent = 'Apuntá al QR de intercambio';
    ...
    stat.textContent = 'No se pudo abrir la cámara — probá con galería o pegá el código';
  }
}
...
async function scanTradeFrame(){
  ...
  try{
    if(video && video.readyState >= 2){
      // BarcodeDetector cuando esté disponible.
      ...
      // jsQR fallback, funciona bien en iPhone/Safari cuando carga la librería.
...
function handleScannedTradeCode(raw){
  ...
  if(parsePayload(payload)){
    ...
    stat.textContent = 'QR leído, pero no es un código de intercambio válido';
...
async function decodeQRFromImage(file){
  ...
  return new Promise((resolve, reject)=>{
    ...
    img.onload = async ()=>{
      ...
      stat.textContent = 'No pude leer el QR — probá acercarte más o pegá el código manual';
