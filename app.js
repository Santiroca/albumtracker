...
const TUTORIAL_KEY = 'pn26_tutorial_seen_v5';
let tutorialStep = 0;
...
function ensureTutorialTargetVisible(target){
  ...
  try{
    ...
    if(isPhone){
      ...
      const reservedBottom = Math.min(320, vp.height * 0.42);
      const visibleTop = reservedTop;
      const visibleBottom = vp.height - reservedBottom;
      ...
      else if(targetCenter < visibleTop || targetCenter > visibleBottom) delta = targetCenter - desiredCenter;
...
function positionTutorial(){
  ...
  if(target){
    ...
    if(isPhone){
      ...
      const cardH = Math.min(card.getBoundingClientRect().height || 260, Math.round(vh*0.44));
      const gap = 12;
      const spaceBelow = vh - (rect.bottom + gap) - (10 + 20); // bottom margin + safe-ish area
      const spaceAbove = rect.top - gap - margin;

      if(spaceBelow >= cardH){
        card.style.top = `${Math.max(margin, rect.bottom + gap)}px`;
        card.style.bottom = 'auto';
      } else if(spaceAbove >= cardH){
        card.style.top = `${Math.max(margin, rect.top - cardH - gap)}px`;
        card.style.bottom = 'auto';
      } else if(spaceBelow >= spaceAbove){
        card.style.top = `${Math.max(margin, Math.min(rect.bottom + gap, vh - cardH - margin))}px`;
        card.style.bottom = 'auto';
      } else {
        card.style.top = `${Math.max(margin, rect.top - cardH - gap)}px`;
        card.style.bottom = 'auto';
