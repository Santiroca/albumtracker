...
async function handlePhotoImportFile(e){
  ...
  toast('Foto cargada. Intentando leer el código...','ok');
  await readPhotoImportCode(file);
}
...
function parsePhotoImportText(text){
  ...
  for(const part of photoImportKnownIdParts()){
    const noPad = `${part.prefix}${part.number}`;
    const onePad = `${part.prefix}${String(part.number).padStart(2, '0')}`;
    if(compactSoft.includes(noPad) || compactSoft.includes(onePad)){
      bestCounts[part.id] = Math.max(bestCounts[part.id] || 0, 1);
    }
  }
...
function buildPhotoOcrAttempts(baseCanvas){
  ...
  function add(name, canvas, mode, psm='6'){
    attempts.push({name, canvas:enhancePhotoCanvas(canvas, mode), psm});
  }

  // Primero intenta imagen completa.
  add('foto completa', baseCanvas, 'contrast', '6');
  add('foto completa nítida', sharpenPhotoCanvas(baseCanvas), 'soft-threshold', '6');

  // Si hay varias figuritas lado a lado o una arriba de otra, los recortes ayudan mucho.
  if(w >= h * 1.15){
    add('mitad izquierda', cropPhotoCanvas(baseCanvas, 0, 0, w * 0.56, h), 'contrast', '6');
    add('mitad derecha', cropPhotoCanvas(baseCanvas, w * 0.44, 0, w * 0.56, h), 'contrast', '6');
  }else if(h >= w * 1.15){
    add('mitad superior', cropPhotoCanvas(baseCanvas, 0, 0, w, h * 0.56), 'contrast', '6');
    add('mitad inferior', cropPhotoCanvas(baseCanvas, 0, h * 0.44, w, h * 0.56), 'contrast', '6');
  }else{
    add('lado izquierdo', cropPhotoCanvas(baseCanvas, 0, 0, w * 0.58, h), 'contrast', '6');
    add('lado derecho', cropPhotoCanvas(baseCanvas, w * 0.42, 0, w * 0.58, h), 'contrast', '6');
  }

  // Pasada más permisiva para textos sueltos si lo anterior no alcanza.
  attempts.push({name:'texto suelto', canvas:enhancePhotoCanvas(baseCanvas, 'gray'), psm:'11'});
...
async function readPhotoImportCode(file){
  ...
  photoImportStatus('Leyendo la foto… acercá la cámara al código y evitá reflejos. No se suma nada sin confirmar.', 'info');

  try{
    const Tesseract = await ensureTesseract();
    const baseCanvas = await loadPhotoImportCanvas(file);
    const attempts = buildPhotoOcrAttempts(baseCanvas);
    ...
    for(let i=0; i<attempts.length; i++){
      ...
      const result = await Tesseract.recognize(attempt.canvas, 'eng', {
        tessedit_pageseg_mode: attempt.psm || '6',
        preserve_interword_spaces: '1',
        tessedit_char_whitelist: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-:.,/ ',
        logger: m => {
          if(m && m.status === 'recognizing text' && Number.isFinite(m.progress)){
            const pct = Math.round(((i + m.progress) / attempts.length) * 100);
            photoImportStatus(`Leyendo la foto… ${pct}% · ${attempt.name}`, 'info');
          }
        }
      });
      ...
      rawJoined += '\n' + rawText;
      const parsed = parsePhotoImportText(rawText);
      parses.push(parsed);
      bestFound = Math.max(bestFound, parsed.found.length);

      // Para 2 o más figuritas, si ya encontró varias en recortes, no fuerza pasadas lentas.
      if(bestFound >= 4 && i >= 2) break;
    }

    // Mezcla todo lo leído para rescatar códigos partidos entre pasadas.
    parses.push(parsePhotoImportText(rawJoined));

    const merged = mergePhotoImportParses(parses);

    if(merged.found.length){
      if(textArea) textArea.value = photoImportParsedToText(merged);
      previewPhotoImport();
      toast(`Detecté ${merged.found.length} código${merged.found.length===1?'':'s'}. Revisalos antes de sumar.`,'ok');
      return;
    }

    photoImportStatus('No pude leer códigos automáticamente. Para que funcione mejor: foto de atrás, códigos grandes, sin reflejo y de a 1–3 figuritas. También podés escribirlos abajo como TUN 18.', 'warn');
...
function previewPhotoImport(){
  ...
  const foundBox = document.getElementById('photo-import-found');
  const applyBtn = document.getElementById('photo-import-apply');
  const parsed = parsePhotoImportText(txt ? txt.value : '');

  pendingPhotoImportIds = parsed.found;

  if(!foundBox || !applyBtn) return;

  if(!parsed.found.length){
    foundBox.style.display = 'block';
    foundBox.innerHTML = '<div class="photo-import-empty">No encontré códigos. Probá escribirlos como ARG 10, MEX-13, CC 4 o FWC 3.</div>';
    applyBtn.style.display = 'none';
    toast('No encontré códigos','warn');
    return;
  }
  ...
  foundBox.style.display = 'block';
  foundBox.innerHTML = `
    <div class="photo-import-title">Se van a sumar ${parsed.found.length} figuritas</div>
    ...
    <div class="photo-import-chips">
      ${unique.map(id => {
        const isDup = (col[id] || 0) > 0;
        return `<span class="photo-chip ${isDup ? 'photo-chip-dup' : 'photo-chip-new'}">${id}${counts[id] > 1 ? ` ×${counts[id]}` : ''}</span>`;
      }).join('')}
    </div>
  ...
  applyBtn.textContent = `Sumar ${parsed.found.length} figurita${parsed.found.length===1?'':'s'} al álbum`;
