...
function savedTextAndDelay(){
  ...
  if(min < 60){
    const rounded = Math.max(1, Math.floor(min / 5) * 5);
...
function renderTeamSummary(){
  ...
  if(teamSummarySort === 'complete' || teamSummarySort === 'missing'){
    ...
    const title = teamSummarySort === 'missing' ? 'Más faltantes' : 'Más completas';
  ...
  function renderChip(id, cls, container, selectedMap, countEl, maxQty){
    ...
    function updateChip(){
      ...
      if(cur === 0){
        ...
        el.textContent = cur > 1 ? `${baseLabel} ×${cur}` : `${baseLabel} ×1`;
      }
...
const TUTORIAL_KEY = 'pn26_tutorial_seen_v7';
let tutorialStep = 0;
...
const tutorialSteps = [
  {
    kicker:'Inicio',
    title:'Bienvenido',
    text:'Guía rápida de la app.',
    target:'.mobile-header',
    desktopTarget:'.sidebar-stats',
    items:[
      ['📊','Progreso','Porcentaje completado.'],
      ['✅','Tengo / Faltan','Cambia al tocar figuritas.']
    ],
    hint:'Tocá Siguiente.'
  },
  {
    ...
    text:'Instalala para usarla como app.',
    hideOnDesktop:true,
    target:'.bottom-nav',
    items:[
      ['🍎','iPhone','Safari → Compartir → Agregar a inicio.'],
      ['🤖','Android','Chrome → ⋮ → Instalar app.'],
      ['⚡','Ventaja','Pantalla completa y acceso rápido.']
    ],
    hint:'Recomendado para iPhone.'
  },
  {
    kicker:'Álbum',
    title:'Marcá figuritas',
    text:'Tocá una figurita para sumarla.',
    before:()=>goTab('album'),
    target:firstVisibleCell,
    items:[
      ['👆','Un toque','Suma una figurita.'],
      ['♻️','Repetida','Otro toque suma repetida.'],
      ['⏱️','Mantener presionado','Mantener: quita una copia.']
    ],
    hint:'Solo mirá el resaltado.'
  },
  {
    kicker:'Álbum',
    title:'Filtros y buscador',
    text:'Filtrá o buscá figuritas.',
    hideOnDesktop:true,
    before:()=>goTab('album'),
    target:()=>document.querySelector('.gfilters') || document.getElementById('gs'),
    items:[
      ['🎛️','Filtros','Todas, Faltan, Tengo, Repetidas.'],
      ['🔎','Buscador','ID, jugador o selección.']
    ],
    hint:'Ej: MEX-13 o Argentina.'
  },
  {
    kicker:'Progreso',
    title:'Progreso por grupos',
    text:'Resumen general y por grupo.',
    before:()=>goTab('summary'),
    target:'#panel-summary',
    items:[
      ['📊','General','Total, tengo, faltan y repetidas.'],
      ['🌎','Por país','Avance de cada país.'],
      ['↕️','Orden','Orden por completas/faltantes.']
    ],
    hint:'Para ver qué completar primero.'
  },
  {
    kicker:'Repetidas',
    title:'Controlá tus repetidas',
    text:'Tus repetidas aparecen acá.',
    before:()=>{
      goTab('dupes');
      renderDupes();
      setTimeout(()=>{
    ...
    items:[
      ['♻️','Lista automática','Cantidad mayor a 1.'],
      ['📋','Copiar lista','Copiá y compartí.']
    ],
    hint:'Si no hay repetidas, queda vacío.'
  },
  {
    kicker:'Intercambio',
    title:'Compartí tu QR',
    text:'Compartí tu QR con amigos.',
    before:()=>goTab('trade'),
    target:'#qrbox',
    items:[
      ['🔁','Mi QR','Tu código de intercambio.'],
      ['📲','WhatsApp','Compartir rápido.']
    ],
    hint:'Sin scrollbar lateral.'
  },
  {
    kicker:'Intercambio',
    title:'Escaneá y compará',
    text:'Escaneá o subí un QR.',
    before:()=>goTab('trade'),
    target:()=>document.querySelector('#panel-trade .btn.btn-p') || document.getElementById('qrfile'),
    items:[
      ['📷','Cámara','Leer QR en vivo.'],
      ['🖼️','Galería','Elegir imagen.'],
      ['🤝','Match','Calcula el match.']
    ],
    hint:'Guardá amigos para después.'
  },
  {
    kicker:'Backup',
    title:'Importar y exportar',
    text:'Backup de tu colección.',
    before:()=>{
      goTab('album');
      setTimeout(()=>{
    ...
    items:[
      ['📤','Exportar','Guarda un archivo.'],
      ['📥','Importar','Carga el backup.']
    ],
    hint:'Podés terminar.'
  }
];
...
function ensureTutorialTargetVisible(target){
  ...
  try{
    ...
    if(isPhone){
      // Keep the highlighted target in a free band that will not be covered by the tutorial card.
      const targetIsLower = rect.top > vp.height * 0.45;
      // If the tutorial card docks at the top, keep the highlight safely below it.
      const freeTop = targetIsLower ? 230 : 110;
      const freeBottom = targetIsLower ? (vp.height - 24) : (vp.height - 210);
      const desiredCenter = (freeTop + freeBottom) / 2;
      const currentCenter = rect.top + rect.height / 2;
      let delta = 0;
      if(rect.top < freeTop) delta = rect.top - freeTop;
      else if(rect.bottom > freeBottom) delta = rect.bottom - freeBottom;
      else if(currentCenter < freeTop || currentCenter > freeBottom) delta = currentCenter - desiredCenter;
      if(Math.abs(delta) > 10){
        if(sc && sc.contains(target)) sc.scrollTo({top: Math.max(0, sc.scrollTop + delta), behavior:'smooth'});
    ...
    const visibleTop = 32;
    const visibleBottom = vp.height - 280;
...
function positionTutorial(){
  ...
  if(target){
    ...
    if(isPhone){
      ...
      const targetIsLower = rect.top > vh * 0.45;
      if(targetIsLower){
