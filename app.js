...
function cellTap(id, el, fromTouch=false){
  // Evita el click sintético de iOS después de un tap real.
  if(!fromTouch && window._lastTouchTap && window._lastTouchTap.id === id && Date.now() - window._lastTouchTap.ts < 800){ return; }
...
function handleCellTouchEnd(e, id, el){
  ...
  if(moved || dx > 10 || dy > 10 || elapsed > 650){
