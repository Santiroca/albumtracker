...
function buildPayload(){
  ...
  // Para que el QR escanee fácil, mandamos la lista más corta:
  // c = figuritas que tiene, m = figuritas que le faltan.
  ...
  return 'p26:' + mainKey + '=' + mainPart + (dPart ? '&d=' + dPart : '');
}
...
function genQR(){
  ...
  try{
    // QR más simple y legible: guardamos el payload directo, no una URL larguísima.
    render(payload);
  }catch(e){
    try{
...
async function startTradeCam(){
  ...
  try{
    ...
    setScannerState('Apuntá al QR de intercambio. Mantenelo quieto dentro del marco.', 'live');
...
function getVideoFrameImageData(video, canvas, mode){
  ...
  // Dos intentos: frame completo y recorte central. El recorte ayuda mucho en iPhone.
  if(mode === 'center'){
    const size = Math.floor(Math.min(canvas.width, canvas.height) * 0.78);
  ...
  try{
    if(video && canvas && video.readyState >= 2){
      ...
      if(window.jsQR){
        ...
        if(window._tradeScanMisses === 90){
          setScannerState('Todavía no lo leo. Acercá el QR, subí el brillo y evitá reflejos.', 'live');
        }
      }else{
