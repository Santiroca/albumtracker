...
const TUTORIAL_KEY = 'pn26_tutorial_seen_v4';
let tutorialStep = 0;
...
function ensureTutorialTargetVisible(target){
  ...
  try{
    ...
    const visibleTop = viewport.top + 40;
    const visibleBottom = viewport.top + viewport.height - 300;
...
function focusTutorialTarget(step){
  ...
  setTimeout(()=>ensureTutorialTargetVisible(currentTutorialTarget(step)), 140);
  setTimeout(scheduleTutorialPosition, 180);
  setTimeout(scheduleTutorialPosition, 420);
  setTimeout(scheduleTutorialPosition, 760);
}
...
function positionTutorial(){
  ...
  const vw = vp.width;
  const vh = vp.height;
  const viewLeft = vp.left;
  const viewTop = vp.top;
  const isPhone = window.innerWidth <= 767;
  ...
  if(target){
    ...
    const left = Math.max(viewLeft + 8, rect.left + viewLeft - pad);
    const top = Math.max(viewTop + 8, rect.top + viewTop - pad);
    ...
    if(isPhone){
      card.style.left = `${viewLeft + margin}px`;
      card.style.width = `${vw - margin*2}px`;
      card.style.top = 'auto';
      card.style.bottom = `max(10px, calc(10px + env(safe-area-inset-bottom)))`;
      return;
    }

    const cardRect = card.getBoundingClientRect();
    const cardW = Math.min(430, vw - margin*2);
    const spaceBelow = (viewTop + vh) - (top + height) - margin;
    const spaceAbove = top - viewTop - margin;
    const placeBelow = spaceBelow >= Math.min(cardRect.height || 260, 310) || spaceBelow >= spaceAbove;

    let cardLeft = Math.min(Math.max(viewLeft + margin, left + width/2 - cardW/2), viewLeft + vw - cardW - margin);
    card.style.width = `${cardW}px`;
    card.style.left = `${cardLeft}px`;

    if(placeBelow){
      card.style.top = `${Math.min(top + height + 12, viewTop + vh - margin - (cardRect.height || 260))}px`;
      card.style.bottom = 'auto';
    } else {
      card.style.bottom = `${Math.min(viewTop + vh - top + 12, vh - margin)}px`;
      card.style.top = 'auto';
    }
  } else {
    spot.classList.remove('visible');
    card.style.width = `min(430px, calc(100vw - 28px))`;
    card.style.left = `${viewLeft + vw/2}px`;
    card.style.top = `${viewTop + vh/2}px`;
