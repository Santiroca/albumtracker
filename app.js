...
const tutorialSteps = [
  ...
  },
  {
    ...
    text:'Si la instalás en la pantalla de inicio, se abre más limpia, ocupa mejor el iPhone y funciona como una app real.',
    hideOnDesktop:true,
    target:'.bottom-nav',
    items:[
      ['🍎','iPhone','Abrila en Safari → tocá Compartir → “Agregar a inicio” → Agregar.'],
      ['🤖','Android','Abrila en Chrome → menú ⋮ → “Instalar app” o “Agregar a pantalla principal”.'],
      ['⚡','Ventaja','La vas a usar a pantalla completa, con mejor formato y acceso directo.']
    ],
    hint:'Si más tarde abrís el link desde el navegador, podés volver a hacer estos pasos cuando quieras.'
  },
  {
    kicker:'Álbum',
...
function ensureTutorialTargetVisible(target){
  ...
  try{
    ...
    const reservedBottom = isPhone ? Math.min(360, viewport.height * .48) : 120;
    const reservedTop = isPhone ? 78 : 24;
    const visibleTop = viewport.top + reservedTop;
    const visibleBottom = viewport.top + viewport.height - reservedBottom;

    if(rect.top < visibleTop || rect.bottom > visibleBottom || rect.height > (visibleBottom - visibleTop)){
...
function positionTutorial(){
  ...
  if(target){
    ...
    const cardW = isPhone ? Math.min(420, vw - margin*2) : Math.min(430, vw - margin*2);
    ...
    if(isPhone){
      ...
      card.style.top = 'auto';
      card.style.bottom = `max(12px, calc(12px + env(safe-area-inset-bottom)))`;
    } else if(placeBelow){
