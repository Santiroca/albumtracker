...
function cellTap(id, el, fromTouch=false){
  // Avoid duplicate synthetic click after a touch tap on iPhone
  if(!fromTouch && window._lastTouchTap && window._lastTouchTap.id === id && Date.now() - window._lastTouchTap.ts < 700){ return; }
...
function handleCellTouchEnd(e, id, el){
  ...
  if(el && el._longFired){ el._longFired = false; return; }
  if(window._lpBlockedId === id){ window._lpBlockedId = null; return; }
  window._lastTouchTap = { id, ts: Date.now() };
  cellTap(id, el, true);
...
(function syncAppHeight(){
  ...
  function setH(){
    /*
      En iOS instalado en inicio, visualViewport puede excluir la zona inferior del
      home indicator y dejar una franja vacía debajo del nav. Para la PWA usamos
      innerHeight, y para Safari normal usamos visualViewport cuando existe.
    */
    const h = isStandaloneIOS()
      ? window.innerHeight
      : ((window.visualViewport && window.visualViewport.height) ? window.visualViewport.height : window.innerHeight);

    document.documentElement.style.setProperty('--app-height', `${Math.round(h)}px`);

    if(window._updateScrollInd) setTimeout(window._updateScrollInd, 60);
    if(window._updateScrollBar) setTimeout(window._updateScrollBar, 60);
  }

  setH();
  window.addEventListener('resize', setH, {passive:true});
  window.addEventListener('orientationchange', ()=>setTimeout(setH, 250), {passive:true});
...
(function(){
  ...
  let trackStart=0, trackEnd=0, trackH=0, barH=0;
  ...
  function calcTrack(){
    ...
    trackEnd   = navTop - 4;
    trackH     = Math.max(80, trackEnd - trackStart);
    const {scrollHeight, clientHeight} = scroll;
    barH = Math.max(window.innerWidth < 768 ? 54 : 40, Math.min(trackH * (clientHeight / scrollHeight), trackH));
  }
  ...
  function updateBar(){
    ...
    if(!isDragging) hideTimer = setTimeout(()=>bar.classList.remove('visible'), window.innerWidth < 768 ? 1800 : 800);
  }
  window._updateScrollBar = updateBar;
  ...
  function updateIndicator(){
    ...
    // Position: follow the scrollbar exactly
    calcTrack(); // ensure trackStart/trackH/barH are current
    const {scrollTop, scrollHeight, clientHeight} = scroll;
    const scrollable = scrollHeight - clientHeight;
    const pct2 = scrollable > 0 ? Math.min(scrollTop / scrollable, 1) : 0;
    const barTop = trackStart + (trackH - barH) * pct2;
    const indH = ind.offsetHeight || 44;
    ind.style.top = (barTop + barH/2 - indH/2) + 'px';
    ind.classList.add('visible');
    clearTimeout(ind._hideTimer);
    ind._hideTimer = setTimeout(()=>ind.classList.remove('visible'), window.innerWidth < 768 ? 1500 : 800);
  }

  // Scroll updates the bar and the team indicator on iPhone too
  ...
  function onDragStart(clientY){
    ...
    updateIndicator(); ind.classList.add('visible');
    clearTimeout(ind._hideTimer);
  }
  ...
  function onDragEnd(){
    isDragging=false; bar.style.transition='opacity .6s ease';
    hideTimer = setTimeout(()=>bar.classList.remove('visible'), window.innerWidth < 768 ? 1800 : 800);
    ind._hideTimer = setTimeout(()=>ind.classList.remove('visible'), window.innerWidth < 768 ? 1500 : 800);
  }
