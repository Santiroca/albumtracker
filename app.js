...
function genQR(){
  ...
  const fullUrl = `https://santiroca.github.io/albumtracker/?code=${encodeURIComponent(payload)}`;
  const dupesOnly = Object.keys(col).filter(id=>col[id]>1).map(encodeShortId).join(',');
  const fallbackUrl = `https://santiroca.github.io/albumtracker/?code=${encodeURIComponent('p26:d=' + dupesOnly)}`;
  ...
  function cleanupQR(){
    ...
    // qrcodejs sometimes creates a hidden IMG fallback next to the canvas.
    // Keep only the real QR node so desktop never shows duplicates.
    direct.forEach(n => { if(n !== qrNode) n.remove(); });

    qrNode.style.display = 'block';
    qrNode.style.margin = '0 auto';
    qrNode.style.width = '220px';
    qrNode.style.height = '220px';
...
async function shareWhatsApp(){
  ...
  canvas.toBlob(async blob => {
    ...
    if(navigator.share && navigator.canShare && navigator.canShare({files:[file]})){
      try{
        await navigator.share({ files: [file], title: 'Mi QR Panini WC 2026' });
...
// ─── FIRST RUN TUTORIAL ─────────────────────────────────
const TUTORIAL_KEY = 'pn26_tutorial_seen_v1';
let tutorialStep = 0;
const tutorialSteps = [
  {
    kicker:'Paso 1 de 5',
    title:'Álbum',
    text:'Tocá una figurita para marcarla como que ya la tenés pegada.',
    items:[
      ['📖','Ver tu álbum','La sección Álbum agrupa todas las figuritas por grupo y selección.'],
      ['✅','Marcar figuritas','Un toque suma la figurita. Si ya la tenías, pasa a repetida.'],
      ['👆','Quitar figuritas','Mantené presionada una card para quitar una copia.']
    ]
  },
  {
    kicker:'Paso 2 de 5',
    title:'Buscar y filtrar',
    text:'Usá los filtros para encontrar rápido lo que necesitás.',
    items:[
      ['🔎','Buscador','Buscá por ID, jugador o país.'],
      ['🎛️','Filtros','Podés ver todas, faltantes, pegadas o repetidas.'],
      ['📊','Progreso','Arriba ves porcentaje, cantidad pegada, faltantes y repetidas.']
    ]
  },
  {
    kicker:'Paso 3 de 5',
    title:'Repetidas',
    text:'La sección Repetidas sirve para preparar intercambios.',
    items:[
      ['♻️','Lista de repetidas','Ves solo las figuritas que tenés más de una vez.'],
      ['📋','Copiar lista','Copiá tus repetidas para pasarlas por WhatsApp.'],
      ['➖','Quitar una','Podés descontar repetidas cuando las cambies.']
    ]
  },
  {
    kicker:'Paso 4 de 5',
    title:'Intercambio',
    text:'Compará tu colección con la de tus amigos.',
    items:[
      ['🔁','Tu QR','Compartí tu QR para que otros comparen con vos.'],
      ['📷','Escanear QR','Escaneá o subí el QR de un amigo.'],
      ['🤝','Match','La app te dice qué das, qué recibís y si conviene.']
    ]
  },
  {
    kicker:'Paso 5 de 5',
    title:'Backup',
    text:'Guardá tu colección para no perder nada.',
    items:[
      ['📤','Exportar','Descargá tu colección como archivo JSON.'],
      ['📥','Importar','Subí ese JSON en otro dispositivo para restaurar.'],
      ['🏠','App en inicio','En iPhone funciona mejor agregada a la pantalla de inicio.']
    ]
  }
];
...
function renderTutorial(){
  ...
  `).join('');
  document.getElementById('tutorial-dots').innerHTML = tutorialSteps.map((_,i)=>`<span class="tutorial-dot ${i===tutorialStep?'active':''}"></span>`).join('');
  document.getElementById('tutorial-next').textContent = tutorialStep === tutorialSteps.length - 1 ? 'Empezar' : 'Siguiente';
  document.getElementById('tutorial-skip').style.visibility = tutorialStep === tutorialSteps.length - 1 ? 'hidden' : 'visible';
}
...
(function initTutorial(){
  const run = () => setTimeout(()=>showTutorial(false), 2300);
