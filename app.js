...
(function(){
  ...
  const wppSrc = 'images/whatsapp-logo.png';
  ...
  if(wppEl && !wppEl.getAttribute('src')) wppEl.src = wppSrc;
})();
...
function updateStats(){
  const validEntries = Object.entries(col).filter(([id,v]) => STICKER_MAP[id] && Number(v) > 0);
  ...
  const teamMap = new Map(
    TEAMS
      ...
      .map(t => {
        ...
        return [t.key, { ...teamDisplayParts(t), key:t.key, got, miss, total, pct }];
      })
  );
...
function onSearchInput(){
  ...
  if(clear && gs) clear.classList.toggle('show', !!gs.value.trim());
  renderGrid();
}
...
function copyDupes(){
  const txt = Object.entries(col).filter(([,v])=>v>1)
    ...
    .map(([id,c])=>`${id} x${c-1}`)
    .join(', ');
  navigator.clipboard.writeText(txt||'Sin repetidas').then(()=>toast('Lista copiada','ok'));
}
...
function performCancelTradeMatch(){
  ...
  const giveCountEl = document.getElementById('give-sel-count');
  const getCountEl  = document.getElementById('get-sel-count');
  if(giveCountEl) giveCountEl.textContent = '0 seleccionadas';
  if(getCountEl) getCountEl.textContent = '0 seleccionadas';
...
function renderFriends(){
  ...
  if(!list) return;
  if(!friends.length){
    list.innerHTML = '<div class="friend-empty empty-state"><div class="empty-title">Todavía no guardaste amigos</div><div class="empty-sub">Escaneá o pegá un código de intercambio y guardá el nombre para encontrarlo rápido la próxima vez.</div></div>';
    return;
  }
...
function stopTradeCam(){
  ...
  const btn = document.getElementById('trade-scan-btn');
  if(btn) btn.textContent = '📷 Escanear QR en vivo';
  if(!window._tradeScanHandled) setScannerState('Listo para escanear intercambio', 'idle');
}
...
function handleScannedTradeCode(raw){
  ...
  if(!parsePayload(payload)){
    ...
    toast('Ese QR no es de intercambio','err');
    return;
  }

  window._tradeScanHandled = true;
  setScannerState('Código detectado. Cargando intercambio…', 'ok');
  stopTradeCam();
...
const TUTORIAL_KEY = 'pn26_tutorial_seen_v8';
let tutorialStep = 0;
...
const tutorialSteps = [
  {
    kicker:'Inicio',
    title:'Tu álbum',
    text:'Marcá y controlá todo rápido.',
    target:'.mobile-header',
    desktopTarget:'.sidebar-stats',
    items:[
      ['✅','Tocá','Sumás figuritas.'],
      ['↩️','Deshacer','Revierte el último toque.']
    ],
    hint:'Guía corta.'
  },
  {
    ...
    hint:'Más cómoda en iPhone.'
  },
  {
    kicker:'Álbum',
    title:'Figuritas',
    text:'Un toque suma. Mantener quita.',
    before:()=>goTab('album'),
    target:firstVisibleCell,
    items:[
      ['👆','Toque','Suma una copia.'],
      ['⏱️','Mantener','Quita una copia.']
    ],
    hint:'Intercambio protege la pegada.'
  },
  {
    kicker:'Buscar',
    title:'Filtros',
    text:'Encontrá figuritas rápido.',
    hideOnDesktop:true,
    before:()=>goTab('album'),
    target:()=>document.querySelector('.gfilters') || document.getElementById('gs'),
    items:[
      ['🎛️','Filtros','Faltan, Tengo, Repetidas.'],
      ['🔎','Buscar','ARG 10, Messi o México.']
    ],
    hint:'La X limpia.'
  },
  {
    kicker:'Progreso',
    title:'Progreso',
    text:'Avance por grupos y países.',
    before:()=>goTab('summary'),
    target:()=>document.querySelector('#panel-summary .team-progress-section') || document.getElementById('panel-summary'),
    items:[
      ['📊','General','Total del álbum.'],
      ['↕️','Orden','Más completas o vacías.']
    ],
    hint:'Scrollbar lateral.'
  },
  {
    kicker:'Repetidas',
    title:'Repetidas',
    text:'Tus copias extra aparecen acá.',
    before:()=>{
      goTab('dupes');
      setTimeout(()=>renderDupes(), 60);
    },
    target:dupesTutorialTarget,
    items:[
      ['♻️','Extras','Cantidad mayor a 1.'],
      ['📋','Copiar','Lista para compartir.']
    ],
    hint:'Si no tenés, queda vacío.'
  },
  {
    kicker:'Intercambio',
    title:'Compartí QR',
    text:'Mostrá tu código.',
    before:()=>goTab('trade'),
    target:'#qrbox',
    items:[
      ['🔳','QR','Lo escanea tu amigo.'],
      ['💬','WhatsApp','También podés mandarlo.']
    ],
    hint:'Carga el match.'
  },
  {
    kicker:'Intercambio',
    title:'Escaneá',
    text:'Compará con un amigo.',
    before:()=>goTab('trade'),
    target:()=>document.getElementById('trade-scan-btn') || document.getElementById('qrfile'),
    items:[
      ['📷','Cámara','Leé el QR.'],
      ['🖼️','Galería','Usá una captura.']
    ],
    hint:'Luego confirmás.'
  },
  {
    kicker:'Backup',
    title:'Backup',
    text:'Guardá o recuperá datos.',
    before:()=>{
      goTab('album');
      setTimeout(()=>{
...
function importStats(collection){
  const entries = Object.entries(collection || {}).filter(([id,v]) => STICKER_MAP[id] && Number(v) > 0);
...
function parseAnyImport(raw){
  ...
  // Texto de Figuritas App: reconstruye usando "Me faltan" + "Repetidas".
  if(/Figuritas App|Me faltan|Repetidas/i.test(text)){
...
function parseFiguritasNumbers(value){
  ...
  // Soporta "1, 2, 3", "1 2 3" y copiados con espacios raros.
...
function parseFiguritasList(raw){
  ...
  if(!text || !/(Figuritas\s+App|Me\s+faltan|Repetidas)/i.test(text)) return null;
  ...
  const missingSection = extractFiguritasSection(text, 'Me\\s+faltan\\s*:?', [
    'Repetidas\\s*:?',
    'Descarga\\s+la\\s+app',
    'https?:\\/\\/'
  ]);
...
// ─── PHOTO ASSISTED IMPORT ───────────────────────────────
// No aplica nada automáticamente: muestra foto + códigos para revisar.
let pendingPhotoImportIds = [];
let photoImportReviewItems = [];

function togglePhotoImport(){
  const box = document.getElementById('photo-import-box');
  if(!box) return;
  box.classList.toggle('show');
  if(box.classList.contains('show')){
    renderPhotoFastAdd();
    setTimeout(()=>document.getElementById('photo-import-text')?.focus(), 120);
  }
}
...
function photoImportIdFromPrefixNumber(prefix, number){
  const p = String(prefix || '').toUpperCase();
  const n = parseInt(number, 10);
  if(!p || !Number.isFinite(n)) return null;
  if(p === 'CC') return `CC${n}`;
  if(p === 'FWC') return `FWC-${n}`;
  return `${p}-${n}`;
}

function photoImportPrefixNumberMap(){
  const map = {};
  for(const rawId of ALL_IDS){
    const id = String(rawId || '').toUpperCase();
    if(id === '00') continue;

    let prefix = '';
    let num = null;
    const cc = id.match(/^CC(\d{1,3})$/);
    const regular = id.match(/^([A-Z]{2,4})-(\d{1,3})$/);

    if(cc){
      prefix = 'CC';
      num = parseInt(cc[1], 10);
    }else if(regular){
      prefix = regular[1];
      num = parseInt(regular[2], 10);
    }
  ...
  Object.keys(map).forEach(prefix => {
    map[prefix] = [...new Set(map[prefix])].sort((a,b)=>a-b);
  });

  return map;
}

function photoImportAppendCode(id){
  const txt = document.getElementById('photo-import-text');
  if(!txt || !id) return;

  const current = String(txt.value || '').trim();
  txt.value = current ? `${current}, ${id}` : id;
  txt.focus();

  const found = document.getElementById('photo-import-found');
  if(found){
    found.style.display = 'block';
    found.innerHTML = `<div class="photo-import-empty">Agregado: <b>${photoImportEscapeHtml(id)}</b>. Tocá más números o confirmá.</div>`;
  }
}

function photoImportRemoveLastCode(){
  const txt = document.getElementById('photo-import-text');
  if(!txt) return;
  const parts = String(txt.value || '')
    .split(',')
    .map(v => v.trim())
    .filter(Boolean);
  parts.pop();
  txt.value = parts.join(', ');
  txt.focus();
}

function photoImportClearTypedCodes(){
  const txt = document.getElementById('photo-import-text');
  if(txt){
    txt.value = '';
    txt.focus();
...
function setPhotoFastPrefix(prefix){
  try{ localStorage.setItem('photoFastPrefix', String(prefix || '').toUpperCase()); }catch(_){}
  renderPhotoFastAdd(String(prefix || '').toUpperCase());
}

function renderPhotoFastAdd(selectedPrefix){
  const wrap = document.getElementById('photo-fast-add');
  if(!wrap) return;

  const map = photoImportPrefixNumberMap();
  const prefixes = Object.keys(map).sort((a,b)=>{
    if(a === 'FWC') return -1;
    if(b === 'FWC') return 1;
    if(a === 'CC') return 1;
    if(b === 'CC') return -1;
  ...
  let saved = '';
  try{ saved = localStorage.getItem('photoFastPrefix') || ''; }catch(_){}
  const prefix = prefixes.includes(selectedPrefix) ? selectedPrefix : (prefixes.includes(saved) ? saved : prefixes[0]);
  const nums = map[prefix] || [];

  wrap.innerHTML = `
    <div class="photo-fast-head">
      <div>
        <div class="photo-fast-title">Carga rápida desde foto</div>
        <div class="photo-fast-help">Elegí el equipo y tocá los números que ves. Si hay repetidas, tocá el mismo número más de una vez.</div>
      </div>
      <select class="photo-fast-select" aria-label="Equipo para carga rápida" onchange="setPhotoFastPrefix(this.value)">
        ${prefixes.map(p => `<option value="${p}" ${p === prefix ? 'selected' : ''}>${p}</option>`).join('')}
      </select>
    </div>
    <div class="photo-fast-grid">
      ${nums.map(n => {
        const id = photoImportIdFromPrefixNumber(prefix, n);
        return `<button type="button" class="photo-fast-num" onclick="photoImportAppendCode('${photoImportEscapeHtml(id)}')">${n}</button>`;
      }).join('')}
    </div>
    <div class="photo-fast-actions">
      <button type="button" class="photo-fast-mini" onclick="photoImportRemoveLastCode()">Borrar último</button>
      <button type="button" class="photo-fast-mini" onclick="photoImportClearTypedCodes()">Vaciar códigos</button>
    </div>
  `;
}


async function handlePhotoImportFile(e){
  const file = e.target.files && e.target.files[0];
  if(!file) return;

  const img = document.getElementById('photo-import-preview');
  if(img){
    const url = URL.createObjectURL(file);
  ...
  pendingPhotoImportIds = [];
  photoImportReviewItems = [];

  const txt = document.getElementById('photo-import-text');
  if(txt) txt.value = '';

  await scanPhotoImportWithVisionApi(file);
}

async function scanPhotoImportWithVisionApi(file){
  const txt = document.getElementById('photo-import-text');
  const foundBox = document.getElementById('photo-import-found');

  photoImportStatus('Escaneando la foto con IA…', 'info');
  if(foundBox){
    foundBox.style.display = 'block';
    foundBox.innerHTML = '<div class="photo-import-empty">Escaneando con IA. Esto puede tardar unos segundos…</div>';
  }
  ...
  try{
    ...
    if(!res.ok){
      const msg = data && data.error ? data.error : 'No pude usar el escáner con IA.';
      throw new Error(msg);
    }

    const items = Array.isArray(data.codes) ? data.codes : [];
    const normalized = items
      .map((item, index) => {
        const code = String(item.id || item.code || '').toUpperCase().trim();
        return {
    ...
    renderPhotoImportVisionReview(normalized, data);

    const detectedText = normalized.map(item => item.code).join(', ');
    if(txt) txt.value = detectedText;

    if(normalized.length){
      photoImportStatus(`IA detectó ${normalized.length} códigos. Revisalos y tocá Confirmar código.`, 'ok');
      toast(`IA detectó ${normalized.length} códigos. Revisá y confirmá.`, 'ok');
    }else{
      photoImportStatus('La IA no encontró códigos claros. Podés escribirlos manualmente abajo.', 'warn');
      if(txt) txt.focus();
    }
  }catch(err){
    console.warn('Escaneo con IA falló', err);
    photoImportStatus('No pude conectar con el escáner con IA. Verificá que el backend esté corriendo y que OPENAI_API_KEY esté configurada. También podés escribir los códigos abajo.', 'warn');
    if(foundBox){
      foundBox.style.display = 'block';
      foundBox.innerHTML = `<div class="photo-import-empty">No pude escanear con IA: ${photoImportEscapeHtml(err.message || 'error desconocido')}. Escribí los códigos manualmente o probá de nuevo.</div>`;
    }
    if(txt) txt.focus();
  }
}

function renderPhotoImportVisionReview(items, data){
  const foundBox = document.getElementById('photo-import-found');
  if(!foundBox) return;

  photoImportReviewItems = items || [];
  foundBox.style.display = 'block';

  const warnings = Array.isArray(data && data.warnings) ? data.warnings : [];
  const rejected = Array.isArray(data && data.rejected) ? data.rejected : [];

  if(!photoImportReviewItems.length){
    foundBox.innerHTML = `
      <div class="photo-import-title">No encontré códigos claros</div>
      <div class="photo-import-empty">Probá con una foto más cercana, bien iluminada y enfocando la esquina superior derecha de las figuritas.</div>
      ${warnings.length ? `<div class="photo-import-warn">${warnings.map(photoImportEscapeHtml).join('<br>')}</div>` : ''}
    `;
    return;
  }

  const okCount = photoImportReviewItems.filter(it => it.confidence === 'high' || it.confidence === 'alta').length;
  foundBox.innerHTML = `
    <div class="photo-import-title">Revisá los códigos detectados por IA</div>
    <div class="photo-import-review-help">
      La IA intenta leer solo la esquina superior derecha de cada figurita. Corregí cualquier error y tocá <b>Confirmar código</b> para sumarlos.
    </div>
    <div class="photo-import-review-list">
      ${photoImportReviewItems.map((item, i) => {
        const confidence = item.confidence || 'medium';
        const isHigh = confidence === 'high' || confidence === 'alta';
        const isLow = confidence === 'low' || confidence === 'baja';
        return `
          <div class="photo-import-review-row">
            <div class="photo-import-ai-index">${i + 1}</div>
            <div class="photo-import-review-main">
              <label class="photo-import-review-label">Código ${i+1}</label>
              <input
                class="photo-import-review-input"
                data-photo-review-index="${i}"
                value="${photoImportEscapeHtml(item.code || '')}"
                placeholder="Ej: TUN 14"
                autocomplete="off"
                autocapitalize="characters"
                spellcheck="false">
              <div class="${isLow ? 'photo-import-review-warn' : 'photo-import-review-ok'}">
                ${isHigh ? 'Confianza alta' : (isLow ? 'Revisar: confianza baja' : 'Revisar: confianza media')}
                ${item.note ? ` · ${photoImportEscapeHtml(item.note)}` : ''}
              </div>
            </div>
          </div>
        `;
    ...
    <div class="photo-import-review-count">${okCount}/${photoImportReviewItems.length} con confianza alta</div>
    ${rejected.length ? `<div class="photo-import-warn">Ignorados por no existir en el álbum: ${rejected.map(photoImportEscapeHtml).join(', ')}</div>` : ''}
    ${warnings.length ? `<div class="photo-import-warn">${warnings.map(photoImportEscapeHtml).join('<br>')}</div>` : ''}
  `;
}


function photoImportKnownPrefixes(){
  const prefixes = new Set();
  for(const id of ALL_IDS){
    const up = String(id || '').toUpperCase();
    if(up === '00') continue;
    const m = up.match(/^([A-Z]{2,4})(?:-|)(\d{1,3})$/);
    if(m) prefixes.add(m[1]);
  }
  return prefixes;
}

function photoImportOcrNumber(raw){
  return String(raw || '')
    .toUpperCase()
    .replace(/[OQ]/g, '0')
    .replace(/[IL|]/g, '1')
...
function photoImportPrefixDistance(a, b){
  a = String(a || '');
  b = String(b || '');
...
function normalizePhotoImportPrefix(prefix){
  const known = photoImportKnownPrefixes();
  ...
  // Casos OCR típicos: FWC puede aparecer como FWG/FVV C y CC como C0/GG.
  const aliases = {
    'FWG':'FWC', 'FVV':'FWC', 'FNC':'FWC', 'FWO':'FWC', 'FW0':'FWC',
    'C0':'CC', 'CO':'CC', 'GG':'CC'
  };
  if(aliases[raw] && known.has(aliases[raw])) return aliases[raw];

  let best = null;
  let bestDist = 99;
  for(const p of known){
    if(Math.abs(p.length - raw.length) > 1) continue;
    const dist = photoImportPrefixDistance(raw, p);
    if(dist < bestDist){
      best = p;
      bestDist = dist;
    }
...
function normalizePhotoImportId(prefix, num){
  const p = normalizePhotoImportPrefix(prefix);
  const digits = photoImportOcrNumber(num);
...
function normalizePhotoImportIdStrict(prefix, num){
  ...
  const digits = photoImportOcrNumber(num);
  const n = parseInt(digits, 10);
  if(!rawPrefix || !Number.isFinite(n)) return null;

  const candidates = [];
  if(rawPrefix === 'CC') candidates.push(`CC${n}`);
  else if(rawPrefix === 'FWC') candidates.push(`FWC-${n}`);
  else if(/^[A-Z]{2,4}$/.test(rawPrefix)) candidates.push(`${rawPrefix}-${n}`);

  return candidates.find(id => known.has(id.toUpperCase())) || null;
}

// Parser conservador para OCR desde FOTO.
// No hace búsqueda compacta de todos los IDs porque eso convierte textos como PANINI o FIFA en figuritas falsas.
function parsePhotoImportOcrStrict(text){
  ...
  function add(prefix, num, counts){
    const id = normalizePhotoImportIdStrict(prefix, num);
    if(id) counts[id] = (counts[id] || 0) + 1;
    else{
      const maybe = normalizePhotoImportId(prefix, num);
      if(maybe) unknown.add(maybe);
    }
  }

  const variants = [
    raw,
    raw.replace(/\b([A-Z])\s+([A-Z])\s+([A-Z])\s*[-:_.]?\s*([0-9OQIL|SBZ]{1,3})\b/g, '$1$2$3 $4'),
    raw.replace(/\b([A-Z]{2,4})([0-9OQIL|SBZ]{1,2})\b/g, '$1 $2')
  ];

  for(const variant of variants){
    const counts = {};
    const re = /(?:^|[^A-Z0-9])((?:C[C0O])|(?:F[WV][CG0O])|(?:[A-Z]{2,4}))[\s\-_.:;\/\\]*([0-9OQIL|SBZ]{1,2})(?=$|[^A-Z0-9])/g;
    let m;
    while((m = re.exec(variant))) add(m[1], m[2], counts);

    // Las variantes son distintas limpiezas del MISMO OCR.
    // No se suman entre sí porque triplican resultados como TUN-14 x6.
    // Se toma la mayor cantidad vista para cada código.
    for(const [id, qty] of Object.entries(counts)){
      bestCounts[id] = Math.max(bestCounts[id] || 0, qty);
    }
  }

  const found = [];
  Object.keys(bestCounts).sort().forEach(id => {
    for(let i=0; i<bestCounts[id]; i++) found.push(id);
  });
  return {found, unknown:[...unknown]};
}

function photoImportKnownIdParts(){
  return ALL_IDS
    .map(id => String(id || '').toUpperCase())
    .map(id => {
      if(id === '00') return null;
      const cc = id.match(/^CC(\d{1,2})$/);
      if(cc) return {id, prefix:'CC', number:String(parseInt(cc[1], 10))};
      const m = id.match(/^([A-Z]{2,4})-(\d{1,3})$/);
      if(!m) return null;
      return {id, prefix:m[1], number:String(parseInt(m[2], 10))};
    })
    .filter(Boolean);
}

function photoImportLooseTextKey(text){
  return String(text || '')
    .normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, '');
...
function parsePhotoImportText(text){
  const known = new Set(ALL_IDS.map(id=>id.toUpperCase()));
  const raw0 = String(text || '')
    .normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .toUpperCase();

  const compactRaw = photoImportLooseTextKey(raw0);
  const compactSoft = compactRaw
    .replace(/FWG/g, 'FWC')
    .replace(/FVV/g, 'FWC')
    .replace(/FNC/g, 'FWC')
    .replace(/C0/g, 'CC')
  ...
  const variants = [
    raw0,
    raw0.replace(/\b([A-Z0-9])\s+([A-Z0-9])\s+([A-Z0-9])\s+([0-9OQIL|SBZ]{1,3})\b/g, '$1$2$3 $4'),
    raw0.replace(/\b([A-Z0-9])\s+([A-Z0-9])\s+([0-9OQIL|SBZ]{1,3})\b/g, '$1$2 $3'),
    // Cuando Tesseract separa letras con saltos/espacios raros: A R G - 10.
    raw0.replace(/\b([A-Z0-9])\s+([A-Z0-9])\s+([A-Z0-9])\s*[-:_.]?\s*([0-9OQIL|SBZ]{1,3})\b/g, '$1$2$3 $4')
  ];

  const bestCounts = {};
  const unknown = new Set();

  function addCandidate(prefix, num, counts){
    const id = normalizePhotoImportId(prefix, num);
    if(!id) return;
    const up = id.toUpperCase();
    if(known.has(up)){
  ...
  for(const raw of variants){
    ...
    // Soporta: ARG 10, ARG-10, ARG10, FWC 3, FWC-3, CC 4, CC4.
    // También tolera errores comunes de OCR en números/letras.
    const re = /(?:^|[^A-Z0-9])((?:C[C0O])|(?:F[WV][CG0O])|(?:[A-Z0-9]{2,4}))[\s\-_.:;\/\\]*([0-9OQIL|SBZ]{1,3})(?=$|[^A-Z0-9])/g;
    let m;
    while((m = re.exec(raw))){
      addCandidate(m[1], m[2], counts);
    }

    // Si reconoce una lista tipo "ARG: 1, 2, 3" también la aprovecha.
    const listRe = /(?:^|[^A-Z0-9])((?:C[C0O])|(?:F[WV][CG0O])|(?:[A-Z0-9]{2,4}))\s*(?:[^\w\d:]{0,8})\s*:\s*([0-9OQIL|SBZ,\s\-_.;]{1,80})/g;
    while((m = listRe.exec(raw))){
      const prefix = m[1];
      const nums = String(m[2] || '').match(/[0-9OQIL|SBZ]{1,3}/g) || [];
      nums.forEach(n => addCandidate(prefix, n, counts));
    }
  ...
  // Último intento: buscar IDs conocidos en el texto compactado.
  // Esto rescata casos como "M EX 13", "ARG10" o "FWC03" cuando no quedaron límites claros.
  // Cuenta repetidos reales, por ejemplo: "TUN14 TUN18 TUN14".
  function compactDigitPattern(digits){
    return String(digits || '').split('').map(ch => {
      if(ch === '0') return '[0OQ]';
      if(ch === '1') return '[1IL|]';
      if(ch === '2') return '[2Z]';
      if(ch === '5') return '[5S]';
  ...
  function countCompactId(prefix, digits){
    if(!prefix || !digits) return 0;
    const safePrefix = String(prefix).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const re = new RegExp(safePrefix + compactDigitPattern(digits) + '(?![0-9OQIL|SBZ])', 'g');
    return (compactSoft.match(re) || []).length;
  }

  for(const part of photoImportKnownIdParts()){
    const noPad = String(part.number);
    const onePad = String(part.number).padStart(2, '0');
    const compactCount = Math.max(countCompactId(part.prefix, noPad), countCompactId(part.prefix, onePad));
    if(compactCount > 0){
      bestCounts[part.id] = Math.max(bestCounts[part.id] || 0, compactCount);
    }
  }

  const found = [];
  Object.keys(bestCounts).forEach(id => {
    for(let i=0; i<bestCounts[id]; i++) found.push(id);
  });
...
function photoImportStatus(html, type='info'){
  const foundBox = document.getElementById('photo-import-found');
  if(!foundBox) return;
  foundBox.style.display = 'block';
  foundBox.innerHTML = `<div class="photo-import-status photo-import-status-${type}">${html}</div>`;
}
...
function loadPhotoImportCanvas(file){
  return new Promise((resolve, reject)=>{
    const img = new Image();
    const url = URL.createObjectURL(file);
    ...
    img.onload = ()=>{
      try{
        // Mantiene suficiente detalle para letras chicas, pero sin hacer explotar el OCR.
        const maxSide = 2600;
        const minSide = 1500;
        let scale = Math.min(1, maxSide / Math.max(img.width, img.height));
        if(Math.max(img.width, img.height) < minSide){
          scale = Math.min(2.4, minSide / Math.max(img.width, img.height));
        }
...
function clonePhotoCanvas(source){
  const c = document.createElement('canvas');
...
function cropPhotoCanvas(source, x, y, w, h){
  const c = document.createElement('canvas');
  ...
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high';
  ctx.drawImage(source, Math.round(x), Math.round(y), Math.round(w), Math.round(h), 0, 0, c.width, c.height);
  return c;
}

function resizePhotoCanvas(source, targetW, targetH){
  const c = document.createElement('canvas');
...
// Detecta directamente la pastilla negra de código de cada figurita, por ejemplo "TUN 14".
// Es más confiable que leer la figurita completa porque evita FIFA, PANINI y textos legales.
function detectPhotoCodePillRects(source){
  const maxSide = 1200;
  ...
  for(let i=0; i<lumas.length; i++){
    const diff = lumas[i] - mean;
    variance += diff * diff;
  }
  const stdev = Math.sqrt(variance / Math.max(1, lumas.length));
  const threshold = Math.max(48, Math.min(92, mean - stdev * 0.25));

  const mask = new Uint8Array(sw * sh);
  for(let i=0; i<lumas.length; i++) mask[i] = lumas[i] <= threshold ? 1 : 0;
  ...
  for(let y=0; y<sh; y++){
    for(let x=0; x<sw; x++){
      ...
      // Pastilla del código: horizontal, compacta y bastante rellena.
      if(
        bw >= minW && bw <= maxW &&
        bh >= minH && bh <= maxH &&
        ratio >= 2.35 && ratio <= 4.10 &&
        fill >= 0.38 &&
        count >= 120
      ){
        const padX = bw * 0.18;
        ...
        rects.push({
          ...
          w: Math.min(source.width, (bw + padX * 2) / scale),
          h: Math.min(source.height, (bh + padY * 2) / scale),
...
function cropPhotoCodePill(source, rect){
  return cropPhotoCanvas(source, rect.x, rect.y, rect.w, rect.h);
}

function buildPhotoCodePillStripCanvas(source, rects){
  const labels = [];
  for(const rect of rects){
    const crop = cropPhotoCodePill(source, rect);
    const targetW = 760;
    const targetH = Math.max(170, Math.round(targetW * crop.height / Math.max(1, crop.width)));
    labels.push(resizePhotoCanvas(crop, targetW, targetH));
  }
  ...
  const pad = 24;
  const w = 760 + pad * 2;
  const h = labels.reduce((sum, c)=>sum + c.height + pad, pad);
...
// Detecta las figuritas como rectángulos claros sobre el fondo.
// No intenta leer toda la figurita: solo sirve para ubicar el código de arriba a la derecha.
function detectPhotoStickerRects(source){
  const maxSide = 900;
  const scale = Math.min(1, maxSide / Math.max(source.width, source.height));
  const sw = Math.max(1, Math.round(source.width * scale));
  const sh = Math.max(1, Math.round(source.height * scale));
  ...
  const stdev = Math.sqrt(variance / Math.max(1, lumas.length));
  const threshold = Math.max(112, Math.min(172, mean + Math.max(18, stdev * 0.55)));
  ...
  const visited = new Uint8Array(sw * sh);
  const rects = [];
  const minPixels = Math.max(180, Math.round(sw * sh * 0.0012));
    ...
    for(let x=0; x<sw; x++){
      ...
      while(stack.length){
        ...
        for(const n of neighbors){
          if(n < 0 || n >= mask.length || visited[n] || !mask[n]) continue;
          const nx = n % sw;
          // Evita saltos de borde entre filas.
          if(Math.abs(nx - cx) > 1) continue;
          visited[n] = 1;
          stack.push(n);
        }
      ...
      // Figurita vertical: suficientemente grande, no demasiado ancha y con bastante área clara.
      if(
        count >= minPixels &&
        bw >= sw * 0.055 &&
        bh >= sh * 0.075 &&
  ...
  // Quita duplicados o rectángulos contenidos.
  const deduped = [];
  rects.sort((a,b)=>b.area-a.area);
  for(const r of rects){
    const duplicate = deduped.some(d => {
      ...
      const inter = ix * iy;
      const smaller = Math.min(r.w*r.h, d.w*d.h);
      return inter / Math.max(1, smaller) > 0.55;
    });
    if(!duplicate) deduped.push(r);
  }
...
function cropPhotoStickerCodeLabel(source, rect){
  // En estas figuritas el código está siempre arriba a la derecha.
  // Recorta un poco más de margen para tolerar rotación o encuadre torcido.
  const x = rect.x + rect.w * 0.45;
  const y = rect.y + rect.h * 0.00;
  const w = rect.w * 0.55;
  const h = rect.h * 0.22;
  return cropPhotoCanvas(source, x, y, w, h);
}

function buildPhotoCodeStripCanvas(source, rects){
  const labels = [];
  for(const rect of rects){
    const crop = cropPhotoStickerCodeLabel(source, rect);
    const targetW = 920;
    const targetH = Math.max(220, Math.round(targetW * crop.height / Math.max(1, crop.width)));
    labels.push(resizePhotoCanvas(crop, targetW, targetH));
  }
...
function enhancePhotoCanvas(source, mode){
  const c = clonePhotoCanvas(source);
  const ctx = c.getContext('2d', { willReadFrequently:true });
  ...
  for(let i=0; i<d.length; i+=4){
    ...
    if(mode === 'gray'){
      ...
      const v = gray > 155 ? 255 : (gray < 95 ? 0 : gray);
      d[i] = d[i+1] = d[i+2] = v;
    }else if(mode === 'invert-threshold'){
      const v = gray > 138 ? 0 : 255;
      d[i] = d[i+1] = d[i+2] = v;
    }
  }
...
function sharpenPhotoCanvas(source){
  const c = clonePhotoCanvas(source);
  const ctx = c.getContext('2d', { willReadFrequently:true });
  ...
  for(let y=0; y<h; y++){
    for(let x=0; x<w; x++){
      ...
      for(let ch=0; ch<3; ch++){
        ...
        d[idx+ch] = Math.max(0, Math.min(255, center - left - right - up - down));
      }
      d[idx+3] = s[idx+3];
    }
  }
...
function buildPhotoOcrAttempts(baseCanvas){
  const attempts = [];

  function add(name, canvas, mode, psm='6', strict=true){
    attempts.push({name, canvas:enhancePhotoCanvas(canvas, mode), psm, strict});
  }

  // 1) Camino principal: detectar la pastilla negra donde está el código.
  // Esto evita que el OCR lea PANINI/FIFA/textos legales y los convierta en figuritas falsas.
  const pillRects = detectPhotoCodePillRects(baseCanvas);
  const pillStrip = buildPhotoCodePillStripCanvas(baseCanvas, pillRects);
  if(pillStrip){
    add(`pastillas de código (${pillRects.length})`, pillStrip, 'invert-threshold', '6', true);
    add(`pastillas de código contraste (${pillRects.length})`, pillStrip, 'contrast', '6', true);
    add(`pastillas de código nítidas (${pillRects.length})`, sharpenPhotoCanvas(pillStrip), 'invert-threshold', '6', true);
    return attempts;
  }

  // 2) Respaldo: si no aparecen las pastillas, intenta detectar la figurita y recortar arriba a la derecha.
  // Sigue siendo conservador: no usa OCR de la foto completa.
  const stickerRects = detectPhotoStickerRects(baseCanvas);
  const codeStrip = buildPhotoCodeStripCanvas(baseCanvas, stickerRects);
  if(codeStrip){
    add(`códigos arriba derecha (${stickerRects.length})`, codeStrip, 'invert-threshold', '6', true);
    add(`códigos arriba derecha contraste (${stickerRects.length})`, codeStrip, 'contrast', '6', true);
    return attempts;
  }

  return attempts;
}
function photoImportParsedToText(parsed){
  const out = [];
  const counts = {};
  for(const id of parsed.found) counts[id] = (counts[id] || 0) + 1;
...
function mergePhotoImportParses(parses){
  const maxCounts = {};
  const unknown = new Set();
  ...
  for(const parsed of parses){
    ...
    for(const id of parsed.found) counts[id] = (counts[id] || 0) + 1;
    for(const [id, qty] of Object.entries(counts)){
      maxCounts[id] = Math.max(maxCounts[id] || 0, qty);
    }
    for(const id of parsed.unknown || []) unknown.add(id);
  }

  const found = [];
  Object.keys(maxCounts).sort().forEach(id => {
    for(let i=0; i<maxCounts[id]; i++) found.push(id);
  });
...
function photoImportEscapeHtml(value){
...
function photoImportCanvasToDataURL(canvas){
  try{
    return canvas.toDataURL('image/jpeg', 0.9);
  }catch(_){
    try{ return canvas.toDataURL('image/png'); }catch(__){ return ''; }
  }
}

function buildPhotoReviewCrops(baseCanvas){
  const out = [];
  const pillRects = detectPhotoCodePillRects(baseCanvas);
  for(const rect of pillRects){
    out.push({
      source:'pill',
      rect,
      canvas:resizePhotoCanvas(cropPhotoCodePill(baseCanvas, rect), 520, Math.max(110, Math.round(520 * rect.h / Math.max(1, rect.w))))
    });
  }

  if(out.length) return out.slice(0, 18);

  const stickerRects = detectPhotoStickerRects(baseCanvas);
  for(const rect of stickerRects){
    const crop = cropPhotoStickerCodeLabel(baseCanvas, rect);
    out.push({
      source:'sticker',
      rect,
      canvas:resizePhotoCanvas(crop, 620, Math.max(140, Math.round(620 * crop.height / Math.max(1, crop.width))))
    });
  }
...
function photoImportBestCodeFromText(text){
  const strict = parsePhotoImportOcrStrict(text || '');
  if(strict.found && strict.found.length) return strict.found[0];

  const loose = parsePhotoImportText(text || '');
  if(loose.found && loose.found.length) return loose.found[0];

  return '';
}

async function readSinglePhotoReviewCrop(Tesseract, crop, index, total){
  const variants = [
    enhancePhotoCanvas(crop.canvas, 'invert-threshold'),
    enhancePhotoCanvas(sharpenPhotoCanvas(crop.canvas), 'invert-threshold'),
    enhancePhotoCanvas(crop.canvas, 'contrast')
  ];
  ...
  for(let i=0; i<variants.length; i++){
    const result = await Tesseract.recognize(variants[i], 'eng', {
      ...
      logger: m => {
        if(m && m.status === 'recognizing text' && Number.isFinite(m.progress)){
          const pct = Math.round(((index + ((i + m.progress) / variants.length)) / Math.max(1, total)) * 100);
          photoImportStatus(`Leyendo recortes… ${pct}%`, 'info');
        }
      }
    });
    ...
    raw += '\n' + text;
    const code = photoImportBestCodeFromText(text);
    if(code){
      best = code;
      break;
    }
  ...
  if(!best) best = photoImportBestCodeFromText(raw);

  return {
    id:`photo-review-${Date.now()}-${index}`,
    code:best,
    image:photoImportCanvasToDataURL(crop.canvas),
    raw:raw.trim(),
    source:crop.source
  };
}

function renderPhotoImportReview(items){
  const foundBox = document.getElementById('photo-import-found');
  if(!foundBox) return;
  ...
  if(!photoImportReviewItems.length){
    foundBox.innerHTML = '<div class="photo-import-empty">No pude ubicar recortes de código. Escribí los códigos manualmente como TUN 18, ARG 10 o FWC 3.</div>';
    return;
  }

  const detected = photoImportReviewItems.filter(it => it.code).length;
  foundBox.innerHTML = `
    <div class="photo-import-title">Revisá cada código detectado</div>
    <div class="photo-import-review-help">
      La app lee solo la zona del código. Corregí los que estén mal y tocá <b>Confirmar código</b> para sumarlos.
    </div>
    <div class="photo-import-review-list">
      ${photoImportReviewItems.map((item, i) => `
        <div class="photo-import-review-row">
          ${item.image ? `<img class="photo-import-review-img" src="${item.image}" alt="Recorte de código ${i+1}">` : ''}
          <div class="photo-import-review-main">
            <label class="photo-import-review-label">Figurita ${i+1}</label>
            <input
              class="photo-import-review-input"
              data-photo-review-index="${i}"
              value="${photoImportEscapeHtml(item.code || '')}"
              ...
              autocapitalize="characters"
              spellcheck="false">
            ${item.code ? '<div class="photo-import-review-ok">Detectado</div>' : '<div class="photo-import-review-warn">No se leyó claro. Escribilo manualmente.</div>'}
          </div>
        </div>
      `).join('')}
    </div>
    <div class="photo-import-review-count">${detected}/${photoImportReviewItems.length} códigos detectados automáticamente</div>
  `;
}

function getPhotoImportReviewText(){
  if(!photoImportReviewItems.length) return '';
  const inputs = [...document.querySelectorAll('.photo-import-review-input')];
  return inputs
...
async function readPhotoImportCode(file){
  const textArea = document.getElementById('photo-import-text');
  pendingPhotoImportIds = [];
  photoImportReviewItems = [];

  photoImportStatus('Buscando la zona del código arriba a la derecha de cada figurita…', 'info');

  try{
    const Tesseract = await ensureTesseract();
    const baseCanvas = await loadPhotoImportCanvas(file);
    const crops = buildPhotoReviewCrops(baseCanvas);

    if(!crops.length){
      photoImportStatus('No pude ubicar los códigos. Probá una foto más cercana, de atrás, con buena luz y sin reflejo. También podés escribirlos abajo como TUN 18.', 'warn');
    ...
    for(let i=0; i<crops.length; i++){
      const item = await readSinglePhotoReviewCrop(Tesseract, crops[i], i, crops.length);
      items.push(item);
    }

    renderPhotoImportReview(items);

    const detectedText = items.map(it => it.code).filter(Boolean).join(', ');
    if(textArea) textArea.value = detectedText;

    const detected = items.filter(it => it.code).length;
    if(detected){
      toast(`Detecté ${detected} de ${items.length}. Revisá y confirmá.`, 'ok');
    }else{
      toast('No pude leer los códigos. Escribilos en los campos detectados.', 'warn');
    }
  }catch(err){
    console.warn('OCR foto falló', err);
    photoImportStatus('No pude activar la lectura automática. Escribí los códigos abajo como TUN 18, ARG 10 o FWC 3.', 'warn');
    if(textArea) textArea.focus();
  }
}


function previewPhotoImport(){
  const txt = document.getElementById('photo-import-text');
  const foundBox = document.getElementById('photo-import-found');
  const reviewText = getPhotoImportReviewText();
  const sourceText = reviewText || (txt ? txt.value : '');
  const parsed = parsePhotoImportText(sourceText);

  pendingPhotoImportIds = parsed.found;

  if(!foundBox) return;

  if(!parsed.found.length){
    foundBox.style.display = 'block';
    foundBox.innerHTML = '<div class="photo-import-empty">No encontré códigos válidos. Escribilos como ARG 10, MEX-13, CC 4 o FWC 3.</div>';
    toast('No encontré códigos válidos','warn');
    return;
  }
  ...
  let newQty = 0;
  let repeatedQty = 0;
  for(const id of parsed.found){
    if((col[id] || 0) > 0) repeatedQty++;
    else newQty++;
  }

  rememberUndo('sumar desde foto');
  ...
  foundBox.style.display = 'block';
  foundBox.innerHTML = `
    <div class="photo-import-title">Se sumaron ${parsed.found.length} figuritas al álbum</div>
    <div class="photo-import-summary">
      <span class="photo-import-pill photo-import-pill-new">${newQty} nueva${newQty===1?'':'s'}</span>
      <span class="photo-import-pill photo-import-pill-dup">${repeatedQty} repetida${repeatedQty===1?'':'s'}</span>
    </div>
    <div class="photo-import-chips">
      ${unique.map(id => {
        const wasDup = ((col[id] || 0) - counts[id]) > 0;
        return `<span class="photo-chip ${wasDup ? 'photo-chip-dup' : 'photo-chip-new'}">${id}${counts[id] > 1 ? ` ×${counts[id]}` : ''}</span>`;
      }).join('')}
    </div>
    ${parsed.unknown.length ? `<div class="photo-import-warn">${parsed.unknown.length} códigos ignorados porque no existen en el álbum: ${parsed.unknown.join(', ')}</div>` : ''}
  `;

  pendingPhotoImportIds = [];
  photoImportReviewItems = [];
  toast(`${parsed.found.length} figuritas sumadas · ${newQty} nuevas · ${repeatedQty} repetidas`, 'ok', {undo:true});
}


function applyPhotoImport(){
  if(!pendingPhotoImportIds.length){
    toast('Primero revisá los códigos','warn');
    return;
  }

  let newQty = 0;
  let repeatedQty = 0;
  for(const id of pendingPhotoImportIds){
    if((col[id] || 0) > 0) repeatedQty++;
    else newQty++;
  }
  ...
  const total = pendingPhotoImportIds.length;
  pendingPhotoImportIds = [];

  const applyBtn = document.getElementById('photo-import-apply');
  if(applyBtn) applyBtn.style.display = 'none';

  toast(`${total} figuritas sumadas · ${newQty} nuevas · ${repeatedQty} repetidas`, 'ok', {undo:true});
}

function clearPhotoImport(){
  pendingPhotoImportIds = [];
  photoImportReviewItems = [];
  const file = document.getElementById('photo-import-file');
  const cameraFile = document.getElementById('photo-camera-file');
  const img = document.getElementById('photo-import-preview');
  const txt = document.getElementById('photo-import-text');
  const found = document.getElementById('photo-import-found');
  const apply = document.getElementById('photo-import-apply');

  if(file) file.value = '';
  if(cameraFile) cameraFile.value = '';
  if(img){ img.removeAttribute('src'); img.style.display = 'none'; }
  if(txt) txt.value = '';
  if(found){ found.innerHTML = ''; found.style.display = 'none'; }
