...
function parsePhotoImportText(text){
  ...
  for(const raw of variants){
    ...
    const re = /(?:^|[^A-Z0-9])((?:C[C0O])|(?:F[WV][CG0O])|(?:[A-Z0-9]{3}))[\s\-_.:;\/\\]*([0-9OQIL|SBZ]{1,3})(?=$|[^A-Z0-9])/g;
    let m;
    while((m = re.exec(raw))){
      const id = normalizePhotoImportId(m[1], m[2]);
      if(!id) continue;
      const up = id.toUpperCase();
      if(known.has(up)){
    ...
    img.onload = ()=>{
      try{
        ...
        if(Math.max(img.width, img.height) < minSide){
          scale = Math.min(2.2, minSide / Math.max(img.width, img.height));
        }
...
function enhancePhotoCanvas(source, mode){
  ...
  for(let i=0; i<d.length; i+=4){
    ...
    if(mode === 'contrast'){
      ...
      const v = gray > 132 ? 255 : 0;
      d[i] = d[i+1] = d[i+2] = v;
    }else if(mode === 'invert-threshold'){
      const v = gray > 132 ? 0 : 255;
      d[i] = d[i+1] = d[i+2] = v;
    }
  }
...
async function readPhotoImportCode(file){
  ...
  photoImportStatus('Leyendo la foto… buscá que los códigos estén claros y ocupen buena parte de la imagen.', 'info');

  try{
    const Tesseract = await ensureTesseract();
    const baseCanvas = await loadPhotoImportCanvas(file);
    const attempts = [
      {name:'contraste', canvas:enhancePhotoCanvas(baseCanvas, 'contrast')},
      {name:'blanco y negro', canvas:enhancePhotoCanvas(baseCanvas, 'threshold')},
      {name:'invertida', canvas:enhancePhotoCanvas(baseCanvas, 'invert-threshold')}
    ];
    ...
    for(let i=0; i<attempts.length; i++){
      ...
      const result = await Tesseract.recognize(attempt.canvas, 'eng', {
        ...
        logger: m => {
          if(m && m.status === 'recognizing text' && Number.isFinite(m.progress)){
            ...
            photoImportStatus(`Leyendo la foto… ${pct}%`, 'info');
          }
        }
      });
      ...
      // Si ya encontró varios códigos, evita una pasada extra lenta.
      if(bestFound >= 3 && i >= 1) break;
    }
    ...
    if(merged.found.length){
      ...
      previewPhotoImport();
      toast(`Detecté ${merged.found.length} código${merged.found.length===1?'':'s'}`,'ok');
      return;
    }

    photoImportStatus('No pude leer códigos automáticamente. Probá una foto más cerca, con buena luz, o escribilos abajo como TUN 18.', 'warn');
