...
function isTradeScannerOpen(){
  return !!(tradStream || _zxingControls);
}
...
async function startTradeCam(){
  ...
  try{
    ...
    // Primero usamos ZXing, que es más sólido en iPhone leyendo QR desde pantallas.
...
function handleIncomingTradeLink(){
  ...
  setTimeout(()=>{
    ...
    if(card) setTimeout(()=>card.scrollIntoView({behavior:'smooth', block:'start'}), 250);
