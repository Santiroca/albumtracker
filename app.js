...
function genQR(){
  ...
  try{
    ...
    try{
      ...
      box.innerHTML = '<div style="color:#111;font-size:.75rem;padding:14px;text-align:center;font-family:Arial,sans-serif;">No se pudo generar el QR.<br>Copiá tu código fijo o compartí por WhatsApp.</div>';
    }
  }
}
...
async function copyTradeLink(){
  try{
    ...
    if(isOnlineSyncConfigured()){
      await ensureOnlineProfileSynced();
      url = buildOnlineFriendUrl();
      refreshMyOnlineCodeUI();
    }else{
      url = buildTradeUrl();
    }
    await navigator.clipboard.writeText(url);
    toast(isOnlineSyncConfigured() ? 'Link fijo copiado' : '¡Link copiado!','ok');
  }catch(e){
    console.error(e);
    toast('No pude copiar el link','err');
  }
}
...
async function shareWhatsApp(){
  try{
    ...
    console.warn('No se pudo sincronizar antes de compartir', e);
  }
  ...
  const shareText = isOnlineSyncConfigured()
    ? `Agregame a Album Tracker con mi código fijo: ${displayOnlineFriendCode(getMyOnlineProfile().id)}\n${shareUrl}`
    : `Escaneá mi QR o abrí este link para intercambiar figuritas:\n${shareUrl}`;
  ...
  if(canvas && navigator.share && navigator.canShare){
    try{
      ...
      // Si el usuario cancela o el navegador no comparte archivos, probamos con link.
    }
  }
  ...
  if(navigator.share){
    try{
      await navigator.share({
        title: 'Mi código Album Tracker',
        text: shareText,
        url: shareUrl
      });
      return;
    }catch(e){
      // Si cancela o falla, usamos WhatsApp Web/app como fallback.
    }
  }

  window.open('https://wa.me/?text=' + encodeURIComponent(shareText), '_blank');
  toast('Abrí WhatsApp para compartir tu código','info');
}
...
async function copyMyOnlineFriendCode(){
  ...
  try{
    ...
    console.error(e);
    toast('No pude crear tu código online','err');
  }
}
...
async function saveOnlineFriend(){
  ...
  try{
    ...
    const parsed = parsePayload(profile.payload);
    if(!parsed) throw new Error('El código online no tiene datos válidos');

    const friends = loadFriends();
    const onlineId = profile.id;
    const existing = friends.findIndex(f =>
      (f.onlineId && f.onlineId === onlineId) ||
    ...
    if(existing >= 0) friends[existing] = {...friends[existing], ...entry};
    ...
    showMatch(parsed);
    document.getElementById('matchcard')?.scrollIntoView({behavior:'smooth',block:'start'});
    toast(`${name} guardado con actualización automática`,'ok');
  }catch(e){
    console.error(e);
...
function renderFriends(){
  ...
  list.innerHTML = friends.map((f,i)=>{
    const badge = f.online ? '<span class="friend-online-badge">AUTO</span>' : '<span class="friend-legacy-badge">VIEJO</span>';
    const dateTxt = f.online
      ? `Código fijo · actualizado ${f.date || ''}`
      : `Código viejo · guardado ${f.date || ''}`;
    return `
    <div class="friend-item ${f.online ? 'friend-online' : 'friend-legacy'}" onclick="loadFriendCode(${i})" title="${f.online ? 'Actualizar repetidas y abrir match' : 'Abrir match guardado'}">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div class="friend-date">${dateTxt}</div>
      </div>
      <div class="friend-actions">
        ${f.online ? `<button class="friend-update" type="button" onclick="updateOnlineFriendFromList(event,${i})">Actualizar y ver</button>` : ''}
        <div class="friend-match-link">${f.online ? 'Abrir actualizado →' : 'Abrir match →'}</div>
...
function saveFriendFromMatch(){
  ...
  if(pendingOnline && pendingOnline.onlineId){
    ...
    const friends = loadFriends();
    const existing = friends.findIndex(f =>
      (f.online && f.onlineId === pendingOnline.onlineId) ||
      f.name.toLowerCase() === name.toLowerCase()
    );
...
async function loadFriendCode(i, opts={}){
  ...
  if(f.online && f.onlineId){
    if(isOnlineSyncConfigured()){
      try{
        toast(`Actualizando repetidas de ${f.name}...`,'info');
...
function deleteFriend(e, i){
  e.stopPropagation();
  const friends = loadFriends();
  const name = friends[i]?.name || 'este amigo';
  const ok = confirm(`¿Querés eliminar a ${name} de tus amigos?`);
  if(!ok) return;
  friends.splice(i,1);
  saveFriends(friends);
  renderFriends();
...
const TUTORIAL_KEY = 'pn26_tutorial_seen_v9';
let tutorialStep = 0;
...
const tutorialSteps = [
  {
    kicker:'Inicio',
    title:'Tu álbum',
    text:'Controlá tus figuritas con toques rápidos.',
    target:'.mobile-header',
    desktopTarget:'.sidebar-stats',
    items:[
      ['✅','Tocá','Suma una figurita.'],
      ['➕','Repetida','Si ya la tenías, aumenta la cantidad.'],
      ['↩️','Deshacer','Revierte el último cambio.']
    ],
    hint:'Guía rápida.'
  },
  {
    ...
    text:'Agregala a la pantalla de inicio.',
    hideOnDesktop:true,
    target:'.bottom-nav',
    items:[
      ['📱','iPhone','Safari → Compartir → Agregar a inicio.'],
      ['🤖','Android','Chrome → Instalar app.'],
      ['🔄','Updates','Si hay cambios, la app te avisa.']
    ],
    hint:'Más cómoda instalada.'
  },
  {
    kicker:'Álbum',
    title:'Cargar figuritas',
    text:'Un toque suma. Mantener presionado quita.',
    before:()=>goTab('album'),
    target:firstVisibleCell,
    items:[
      ['👆','Toque','Suma una copia.'],
      ['⏱️','Mantener','Quita una copia.'],
      ['🛡️','Seguro','Los intercambios actualizan el álbum por vos.']
    ],
    hint:'No necesitás guardar: queda automático.'
  },
  {
    kicker:'Buscar',
    title:'Filtros y buscador',
    text:'Encontrá rápido una figurita.',
    hideOnDesktop:true,
    before:()=>goTab('album'),
    target:()=>document.querySelector('.gfilters') || document.getElementById('gs'),
    items:[
      ['🎛️','Filtros','Faltan, Tengo o Repetidas.'],
      ['🔎','Buscar','Ej: ARG 10, Messi, México.'],
      ['✖️','Limpiar','La X borra la búsqueda.']
    ],
    hint:'Ideal cuando el álbum crece.'
  },
  {
    kicker:'Progreso',
    title:'Tu avance',
    text:'Mirá cómo venís por grupos y países.',
    ...
    items:[
      ['📊','General','Completadas, faltantes y repetidas.'],
      ['🌎','Países','Ves qué equipos te faltan más.'],
      ['↕️','Orden','Podés ordenar el resumen.']
    ],
    hint:'Sirve para decidir qué buscar.'
  },
  {
    kicker:'Repetidas',
    title:'Tus extras',
    text:'Acá aparecen las figuritas que tenés de más.',
    before:()=>{
      goTab('dupes');
    ...
    items:[
      ['♻️','Repetida','Cantidad mayor a 1.'],
      ['📋','Copiar','Copiá tu lista de repetidas.'],
      ['🤝','Intercambio','Se usan para calcular matches.']
    ],
    hint:'Si no tenés extras, queda vacío.'
  },
  {
    kicker:'Intercambio',
    title:'Tu código fijo',
    text:'Este es el código que pasás una sola vez.',
    before:()=>goTab('trade'),
    target:'#qrbox',
    items:[
      ['🔳','QR','Tu amigo lo escanea y te guarda.'],
      ['💬','WhatsApp','También podés compartirlo por mensaje.'],
      ['🔗','Link','Copiar link fijo hace lo mismo.']
    ],
    hint:'Cuando cambies repetidas, tus amigos ven la actualización.'
  },
  {
    kicker:'Intercambio',
    title:'Agregar un amigo',
    text:'Escaneá el QR o pegá su código fijo.',
    before:()=>goTab('trade'),
    target:()=>document.getElementById('trade-scan-btn') || document.querySelector('.online-add-row'),
    items:[
      ['📷','Escanear QR','Leé el QR fijo de tu amigo.'],
      ['🖼️','Galería','También sirve una captura.'],
      ['⌨️','Código','Pegalo manualmente si te lo mandan.']
    ],
    hint:'Lo guardás una vez.'
  },
  {
    kicker:'Intercambio',
    title:'Mis amigos',
    text:'Cada amigo queda guardado con su código fijo.',
    before:()=>goTab('trade'),
    target:()=>document.querySelector('.trade-friends-card') || document.getElementById('friend-list'),
    items:[
      ['🔄','Actualizar y ver','Trae sus repetidas nuevas y abre el match.'],
      ['👆','Tarjeta','Tocarla también actualiza y abre.'],
      ['✖️','Eliminar','La X pide confirmación antes de borrar.']
    ],
    hint:'No tenés que pedirle el código otra vez.'
  },
  {
    kicker:'Intercambio',
    title:'Confirmar cambio',
    text:'Elegí qué das y qué recibís.',
    before:()=>goTab('trade'),
    target:()=>document.getElementById('matchcard') || document.querySelector('.trade-step-3') || document.getElementById('panel-trade'),
    items:[
      ['🎯','Match','La app muestra qué conviene intercambiar.'],
      ['✅','Confirmar','Actualiza tu álbum automáticamente.'],
      ['🚫','Cancelar','No modifica nada.']
    ],
    hint:'Confirmá solo cuando el intercambio esté cerrado.'
  },
  {
    kicker:'Backup',
    title:'Importar y exportar',
    text:'Guardá una copia o pasá datos desde otra app.',
    before:()=>{
      goTab('album');
      setTimeout(()=>{
    ...
    items:[
      ['📤','Exportar','Descargá tu backup.'],
      ['📥','Importar','Pegá datos con vista previa.'],
      ['🟢','Confirmar','Revisás antes de aplicar.']
    ],
    hint:'Listo.'
  }
]
