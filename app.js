...
async function copyTradeLink(){
  try{
    ...
    toast(isOnlineSyncConfigured() ? 'Link fijo copiado. Al abrirlo carga el código en la app.' : '¡Link copiado!','ok');
  }catch(e){
    console.error(e);
...
async function shareQRImage(){
  ...
  canvas.toBlob(async blob=>{
    if(!blob){ toast('No se pudo generar la imagen del QR','warn'); return; }
    ...
    const shareText = isOnlineSyncConfigured()
      ? `Agregame a Album Tracker escaneando este QR. Mi código fijo es ${displayOnlineFriendCode(getMyOnlineProfile().id)}.`
    ...
    if(navigator.share && navigator.canShare && navigator.canShare({files:[file]})){
      try{
        ...
        // Si cancela o falla, descargamos el QR.
      }
    }
    ...
    a.download = 'albumtracker-qr.png';
    a.click();
    toast('Se descargó el QR para compartirlo','ok');
  }, 'image/png');
}
...
async function copyMyOnlineFriendLink(){
  ...
  try{
    ...
    if(!(await copyTextToClipboard(link))) throw new Error('copy_failed');
    toast('Link de código fijo copiado','ok');
  }catch(e){
    console.error(e);
    toast('No pude crear tu link online','err');
  }
}
...
function renderFriends(){
  ...
  list.innerHTML = friends.map((f,i)=>{
    const badge = f.online ? '' : '<span class="friend-legacy-badge">VIEJO</span>';
    ...
    <div class="friend-item ${f.online ? 'friend-online' : 'friend-legacy'}" onclick="loadFriendCode(${i})" title="${f.online ? 'Actualizar repetidas y abrir match' : 'Abrir match guardado'}">
      ...
      <div class="friend-actions">
        <button class="friend-update" type="button" onclick="updateOnlineFriendFromList(event,${i})">${f.online ? 'Actualizar y ver repetidas' : 'Abrir match'}</button>
...
const tutorialSteps = [
  ...
  },
  {
    ...
    items:[
      ...
      ['📤','Compartir QR','Compartí la imagen del QR desde el botón.']
    ],
  ...
  },
  {
    ...
    items:[
      ['🔄','Actualizar y ver repetidas','Trae sus repetidas nuevas y abre el match.'],
      ['👆','Tarjeta','Tocarla también actualiza y abre.'],
      ['✖️','Eliminar','La X abre una confirmación dentro de la app antes de borrar.']
    ],
