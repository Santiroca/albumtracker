...
function copyDupes(){
  const txt = Object.entries(col).filter(([,v])=>v>1)
    ...
    .map(([id,c])=>`${id} x${c-1}`)
    .join(', ');
  copyTextToClipboard(txt||'Sin repetidas').then(ok=>toast(ok ? 'Lista copiada' : 'No pude copiar la lista', ok ? 'ok' : 'err'));
}
...
function renderTradeLiveSummary(){
  ...
  getEl.innerHTML = rows(window._tradeGetSelected, window._tradeGet, 'Todavía no elegiste ninguna');
  giveEl.innerHTML = rows(window._tradeGiveSelected, window._tradeGive, 'Todavía no elegiste ninguna');
}
...
function refreshMyOnlineCodeUI(){
  ...
  setOnlineStatus('Compartí este código una sola vez. La app sube tus repetidas cuando cambiás tu álbum.', 'ok');
}
...
// ─── FRIENDS ─────────────────────────────────────────────
const LAST_OPEN_FRIEND_KEY = 'pn26_last_open_friend';

function rememberLastOpenFriend(friend){
  if(!friend) return;
  try{
    const payload = friend.online && friend.onlineId
      ? { type:'online', onlineId: friend.onlineId }
      : { type:'local', name: friend.name || '', code: friend.code || '' };
    localStorage.setItem(LAST_OPEN_FRIEND_KEY, JSON.stringify(payload));
  }catch(_){}
}

function findLastOpenFriendIndex(friends){
  try{
    const saved = JSON.parse(localStorage.getItem(LAST_OPEN_FRIEND_KEY) || 'null');
    if(!saved) return -1;
    if(saved.type === 'online' && saved.onlineId){
      return friends.findIndex(f => f.online && f.onlineId === saved.onlineId);
    }
    if(saved.type === 'local'){
      return friends.findIndex(f => (f.code || '') === saved.code && (f.name || '') === saved.name);
    }
  }catch(_){}
  return -1;
}

function restoreLastOpenFriend(){
  if(isActive('trade')) return;
  const friends = loadFriends();
  const idx = findLastOpenFriendIndex(friends);
  if(idx < 0) return;
  setTimeout(() => {
    goTab('trade');
    loadFriendCode(idx, {silent:true, restoring:true});
  }, 650);
}
...
function renderFriends(){
  ...
  list.innerHTML = friends.map((f,i)=>{
    ...
    const dateTxt = f.online
      ? ''
      : `Guardado ${f.date || ''}`;
    const benefitCount = friendBenefitCountFromPayload(f.code);
    ...
    <div class="friend-item ${f.online ? 'friend-online' : 'friend-legacy'}" onclick="loadFriendCode(${i})" title="${f.online ? 'Actualizar repetidas y abrir match' : 'Abrir match guardado'}">
      ...
      <div class="friend-info">
        <div class="friend-mainline">
          ...
          <div class="friend-date ${benefitClass}">${benefitTxt}</div>
        </div>
        ${dateTxt ? `<div class="friend-date">${dateTxt}</div>` : ''}
      </div>
...
const hasIncomingTradeLink = handleIncomingTradeLink();
if(!hasIncomingTradeLink){
  restoreLastOpenFriend();
}
...
const TUTORIAL_KEY = 'pn26_tutorial_seen_v11';
let tutorialStep = 0;
...
const tutorialSteps = [
  {
    kicker:'Paso 1',
    title:'Bienvenido a tu álbum',
    text:'La app guarda tu colección en este dispositivo y te ayuda a saber qué tenés, qué te falta y qué podés cambiar.',
    target:'.mobile-header',
    desktopTarget:'.sidebar-stats',
    items:[
      ['✅','Tocá una figurita','Suma una copia a tu álbum.'],
      ['♻️','Repetidas','Cuando tenés 2 o más, la app la cuenta como repetida.'],
      ['↩️','Deshacer','Si te equivocás, podés volver atrás el último cambio.']
    ],
    hint:'Todo se guarda automáticamente.'
  },
  {
    kicker:'Paso 2',
    title:'Instalar como app',
    text:'Para que quede como una app, agregala a la pantalla de inicio. En iPhone tiene que ser desde Safari.',
    hideOnDesktop:true,
    target:'.bottom-nav',
    items:[
      ['1️⃣','iPhone: abrí Safari','Entrá a la app desde Safari. No uses Instagram, WhatsApp ni Chrome para instalarla.'],
      ['2️⃣','Tocá Compartir','Es el ícono del cuadrado con una flecha hacia arriba, abajo en Safari.'],
      ['3️⃣','Elegí Agregar a inicio','Buscá “Agregar a pantalla de inicio” en el menú. Si no aparece, deslizá las opciones.'],
      ['4️⃣','Confirmá','Tocá “Agregar”. Va a aparecer el ícono de la app en tu pantalla de inicio.'],
      ['🤖','Android','Abrí en Chrome → menú ⋮ → Instalar app o Agregar a pantalla principal.'],
      ['🔄','Actualizaciones','Si hay cambios nuevos, la app te muestra un aviso para actualizar.']
    ],
    hint:'En iPhone, si tocás Actualizar y sigue igual, cerrá la app desde multitarea y abrila de nuevo desde el ícono.'
  },
  {
    kicker:'Paso 3',
    title:'Cargar figuritas',
    text:'En la pestaña Álbum cargás tu colección con toques rápidos.',
    before:()=>goTab('album'),
    target:firstVisibleCell,
    items:[
      ['👆','Un toque','Agrega una copia de esa figurita.'],
      ['⏱️','Mantener presionado','Quita una copia si cargaste de más.'],
      ['🔢','Cantidad','La tarjeta cambia según si no la tenés, la tenés o está repetida.']
    ],
    hint:'No hay botón guardar: se guarda solo.'
  },
  {
    kicker:'Paso 4',
    title:'Buscar y filtrar',
    text:'Usá el buscador para encontrar una figurita sin recorrer todo el álbum.',
    hideOnDesktop:true,
    before:()=>goTab('album'),
    target:()=>document.querySelector('.gfilters') || document.getElementById('gs'),
    items:[
      ['🔎','Buscar','Probá con ARG 10, TUN, Messi, México o un número.'],
      ['🎛️','Filtros','Podés ver Todas, Faltan, Tengo o Repetidas.'],
      ['✖️','Limpiar','La X borra la búsqueda y vuelve a mostrar todo.']
    ],
    hint:'Es útil cuando ya cargaste muchas figuritas.'
  },
  {
    kicker:'Paso 5',
    title:'Ver tu progreso',
    text:'En Resumen ves cómo viene tu álbum completo y por equipo.',
    ...
    items:[
      ['📊','General','Muestra cuántas tenés, cuántas faltan y cuántas son repetidas.'],
      ['🌎','Por equipo','Sirve para ver qué país o grupo te falta completar.'],
      ['↕️','Ordenar','Podés ordenar para encontrar rápido lo más importante.']
    ],
    hint:'Miralo antes de intercambiar para saber qué necesitás.'
  },
  {
    kicker:'Paso 6',
    title:'Tus repetidas',
    text:'En Repetidas aparecen solo las figuritas que tenés de más.',
    before:()=>{
      goTab('dupes');
    ...
    items:[
      ['♻️','Qué aparece','Una figurita aparece acá cuando tenés 2 o más.'],
      ['📋','Copiar lista','Podés copiar tus repetidas para mandarlas.'],
      ['🤝','Intercambios','La app usa estas repetidas para calcular qué le podés dar a un amigo.']
    ],
    hint:'Si todavía no tenés repetidas, esta sección queda vacía.'
  },
  {
    kicker:'Paso 7',
    title:'Tu código fijo',
    text:'En Intercambio tenés un código fijo. Se lo pasás a un amigo una sola vez.',
    before:()=>goTab('trade'),
    target:'#qrbox',
    items:[
      ['🔳','QR','Tu amigo puede escanear tu QR para guardarte.'],
      ['🟢','Compartir','El botón Compartir abre el menú del celular para mandar tu QR.'],
      ['🔗','Copiar link','También podés copiar un link que abre la app con tu código listo.']
    ],
    hint:'Cuando vos cambies tus repetidas, tu amigo puede verlas actualizadas.'
  },
  {
    kicker:'Paso 8',
    title:'Agregar un amigo',
    text:'Para guardar a alguien, escaneá su QR o pegá su código fijo.',
    before:()=>goTab('trade'),
    target:()=>document.getElementById('trade-scan-btn') || document.querySelector('.online-add-row'),
    items:[
      ['📷','Escanear QR','Usá la cámara para leer el QR fijo de tu amigo.'],
      ['🖼️','Elegir de galería','También sirve una captura del QR.'],
      ['⌨️','Manual','Poné nombre, pegá el código fijo y tocá Guardar amigo.']
    ],
    hint:'Si el código no existe, la app te avisa.'
  },
  {
    kicker:'Paso 9',
    title:'Mis amigos',
    text:'Los amigos guardados aparecen en una lista. No hace falta pedirles el código otra vez.',
    before:()=>goTab('trade'),
    target:()=>document.querySelector('.trade-friends-card') || document.getElementById('friend-list'),
    items:[
      ['🔵','Actualizar y ver repetidas','Trae sus repetidas nuevas y abre el detalle del intercambio.'],
      ['🟢','Tiene X que te sirven','Te muestra rápido si ese amigo tiene figuritas que te faltan.'],
      ['✖️','Eliminar','La X pide confirmación dentro de la app antes de borrar.']
    ],
    hint:'Si tu amigo actualiza su álbum, tocá Actualizar y ver repetidas.'
  },
  {
    kicker:'Paso 10',
    title:'Ver el intercambio',
    text:'La app compara tu álbum con las repetidas de tu amigo y te muestra qué conviene cambiar.',
    before:()=>goTab('trade'),
    target:()=>document.getElementById('matchcard') || document.querySelector('.trade-step-3') || document.getElementById('panel-trade'),
    items:[
      ['🎯','Lo que te sirve','Figuritas repetidas de tu amigo que a vos te faltan.'],
      ['🤝','Lo que le sirve','Tus repetidas que a tu amigo le pueden faltar.'],
      ['✅','Confirmar','Cuando hicieron el cambio, confirmás y tu álbum se actualiza.']
    ],
    hint:'Confirmá solo cuando el intercambio ya esté cerrado.'
  },
  {
    kicker:'Paso 11',
    title:'Backup e importación',
    text:'Hacé una copia de seguridad para no perder tu álbum o para pasarlo a otro dispositivo.',
    before:()=>{
      goTab('album');
      setTimeout(()=>{
    ...
    items:[
      ['📤','Exportar backup','Descargá una copia de tu álbum.'],
      ['📥','Importar','Pegá o cargá un backup para recuperar datos.'],
      ['🟢','Revisar antes','La app muestra una vista previa antes de aplicar cambios.']
    ],
    hint:'Recomendado antes de cambiar de celular.'
  }
]
...
async function checkPwaAssetUpdate({forceNotice=false} = {}){
  try{
    const signature = await getPublishedAssetSignature();
    latestPwaSignature = signature;

    const saved = localStorage.getItem(PWA_SIGNATURE_KEY);
    ...
    if(saved !== signature){
      if(forceNotice || !sessionStorage.getItem('pn26_pwa_update_dismissed')){
        showPwaUpdateNotice();
      }
      return true;
    }
